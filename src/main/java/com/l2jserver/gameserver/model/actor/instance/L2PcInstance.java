...
public final class L2PcInstance extends L2Playable {
	...
	private int _lastCompassZone; // the last compass zone update send to the
									// client
	...
	private final Set<L2PcInstance> _snoopListener = ConcurrentHashMap
			.newKeySet(1);
	private final Set<L2PcInstance> _snoopedPlayer = ConcurrentHashMap
			.newKeySet(1);
	...
	private volatile EnumIntBitmask<ClanPrivilege> _clanPrivileges = new EnumIntBitmask<>(
			ClanPrivilege.class, false);
	...
	public boolean isSpawnProtected() {
		return _protectEndTime > GameTimeController.getInstance()
				.getGameTicks();
	}
	...
	public boolean isTeleportProtected() {
		return _teleportProtectEndTime > GameTimeController.getInstance()
				.getGameTicks();
	}
	...
	/** Player's cubics. */
	private final Map<Integer, L2CubicInstance> _cubics = new ConcurrentSkipListMap<>(); // TODO(Zoey76):
																							// This
																							// should
																							// be
																							// sorted
	...
	private final int[] _htmlActionOriginObjectIds = new int[HtmlActionScope
			.values().length];
	/**
	...
	private final LinkedList<String>[] _htmlActionCaches = new LinkedList[HtmlActionScope
			.values().length];
	...
	/**
	 * Current skill in use. Note that L2Character has _lastSkillCast, but this
	 * has the button presses
	 */
	...
	private L2PcInstance(int objectId, L2PcTemplate template,
			String accountName, PcAppearance app) {
	 ...
	 *            the player appearance
	 */
	private L2PcInstance(L2PcTemplate template, String accountName,
			PcAppearance app) {
	...
	public void startPvPFlag() {
		...
		if (_PvPRegTask == null) {
			_PvPRegTask = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new PvPFlagTask(this), 1000,
							1000);
		}
	}
	 ...
	 */
	public static L2PcInstance create(L2PcTemplate template,
			String accountName, String name, PcAppearance app) {
	...
	public int getRelation(L2PcInstance target) {
			...
			for (int i = 0; i < getParty().getMembers().size(); i++) {
				...
				case 2:
					result |= RelationChanged.RELATION_PARTY3
							+ RelationChanged.RELATION_PARTY2
		...
		if (getSiegeState() != 0) {
			if (TerritoryWarManager.getInstance()
					.getRegisteredTerritoryId(this) != 0) {
		...
		if ((getClan() != null) && (target.getClan() != null)) {
			if ((target.getPledgeType() != L2Clan.SUBUNIT_ACADEMY)
					...
					&& target.getClan().isAtWarWith(getClan().getId())) {
		...
		if (getBlockCheckerArena() != -1) {
			...
			ArenaParticipantsHolder holder = HandysBlockCheckerManager
					.getInstance().getHolder(getBlockCheckerArena());
	 ...
	 * it in _allObjects of the L2world (call restore method).<br>
	 * <B><U> Actions</U> :</B>
	 * <ul>
	...
	public void setTemplate(ClassId newclass) {
		super.setTemplate(PlayerTemplateData.getInstance()
				.getTemplate(newclass));
	}
	 ...
	 * 
	 * @param newbieRewards
	 *            The Identifier of the _newbie state
	 */
	...
	public L2RecipeList[] getCommonRecipeBook() {
		return _commonRecipeBook.values().toArray(
				new L2RecipeList[_commonRecipeBook.values().size()]);
	}
	...
	public L2RecipeList[] getDwarvenRecipeBook() {
		return _dwarvenRecipeBook.values().toArray(
				new L2RecipeList[_dwarvenRecipeBook.values().size()]);
	}
	...
	/**
	 * @param recipeId
	 *            The Identifier of the L2RecipeList to check in the player's
	 *            recipe books
	 * @return {@code true}if player has the recipe on Common or Dwarven Recipe
	 *         book else returns {@code false}
	 */
	public boolean hasRecipeList(int recipeId) {
		return _dwarvenRecipeBook.containsKey(recipeId)
	...
	public void unregisterRecipeList(int recipeId) {
		...
		for (Shortcut sc : getAllShortCuts()) {
			if ((sc != null) && (sc.getId() == recipeId)
					&& (sc.getType() == ShortcutType.RECIPE)) {
	...
	private void insertNewRecipeData(int recipeId, boolean isDwarf) {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("INSERT INTO character_recipebook (charId, id, classIndex, type) values(?,?,?,?)")) {
			...
			LOG.warn("SQL exception while inserting recipe: {} from player {}",
					recipeId, this, e);
		}
	}
	...
	private void deleteRecipeData(int recipeId, boolean isDwarf) {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("DELETE FROM character_recipebook WHERE charId=? AND id=? AND classIndex=?")) {
			ps.setInt(1, getObjectId());
			ps.setInt(2, recipeId);
			ps.setInt(3, isDwarf ? _classIndex : 0);
			ps.execute();
		} catch (SQLException e) {
			LOG.warn("SQL exception while deleting recipe: {} from player {}",
					recipeId, this, e);
		}
	}
	 ...
	 * 
	 * @param quest
	 *            the quest state to check
	 * @return {@code true} if the player has the quest state, {@code false}
	 *         otherwise
	 */
	...
	public void processQuestEvent(String questName, String event) {
		...
		if (getLastQuestNpcObject() > 0) {
			final L2Object object = L2World.getInstance().findObject(
					getLastQuestNpcObject());
			if (object.isNpc()
					&& isInsideRadius(object, L2Npc.INTERACTION_DISTANCE,
							false, false)) {
	 ...
	 * L2PcInstance's or its pet's death
	 */
	...
	/**
	 * Remove QuestState instance that is to be notified of L2PcInstance's
	 * death.
	 * 
	 * @param qs
	 *            The QuestState that subscribe to this event
	...
	public final boolean isNotifyQuestOfDeathEmpty() {
		return (_notifyQuestOfDeathList == null)
				|| _notifyQuestOfDeathList.isEmpty();
	}
	...
	/**
	 * @param slot
	 *            The slot in which the shortCuts is equipped
	 * @param page
	 ...
	 * 
	 * @param skillId
	 *            the skill Id to search and update.
	 * @param skillLevel
	 *            the skill level to update.
	 */
	...
	/**
	 * @param macro
	 *            the macro to add to this L2PcInstance.
	 */
	...
	public void updatePvPFlag(int value) {
		...
		if (hasSummon()) {
			sendPacket(new RelationChanged(getSummon(), getRelation(this),
					false));
		}

		Collection<L2PcInstance> plrs = getKnownList().getKnownPlayers()
				.values();

		for (L2PcInstance target : plrs) {
			target.sendPacket(new RelationChanged(this, getRelation(target),
					isAutoAttackable(target)));
			if (hasSummon()) {
				target.sendPacket(new RelationChanged(getSummon(),
						getRelation(target), isAutoAttackable(target)));
			}
		}
	}
	...
	public void revalidateZone(boolean force) {
		...
		if (isInsideZone(ZoneId.ALTERED)) {
			...
			ExSetCompassZoneCode cz = new ExSetCompassZoneCode(
					ExSetCompassZoneCode.ALTEREDZONE);
			sendPacket(cz);
			...
			ExSetCompassZoneCode cz = new ExSetCompassZoneCode(
					ExSetCompassZoneCode.SIEGEWARZONE2);
			sendPacket(cz);
			...
			ExSetCompassZoneCode cz = new ExSetCompassZoneCode(
					ExSetCompassZoneCode.PEACEZONE);
			sendPacket(cz);
		} else {
			...
			ExSetCompassZoneCode cz = new ExSetCompassZoneCode(
					ExSetCompassZoneCode.GENERALZONE);
			sendPacket(cz);
		}
	}
	...
	public void setPkKills(int pkKills) {
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerPKChanged(this, _pkKills, pkKills), this);
		_pkKills = pkKills;
	}
	...
	/**
	 * Increment the number of recommendation obtained by the L2PcInstance (Max
	 * : 255).
	 */
	...
	/**
	 * Set the number of recommendation obtained by the L2PcInstance (Max :
	 * 255).
	 * 
	 * @param value
	 */
	...
	public void setKarma(int karma) {
		// Notify to scripts.
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerKarmaChanged(this, getKarma(), karma), this);

		if (karma < 0) {
			karma = 0;
		}
		if ((_karma == 0) && (karma > 0)) {
			Collection<L2Object> objs = getKnownList().getKnownObjects()
					.values();
			...
			for (L2Object object : objs) {
				...
				if (((L2GuardInstance) object).getAI().getIntention() == CtrlIntention.AI_INTENTION_IDLE) {
					((L2GuardInstance) object).getAI().setIntention(
							CtrlIntention.AI_INTENTION_ACTIVE, null);
				}
			}
		} else if ((_karma > 0) && (karma == 0)) {
			// Send a Server->Client StatusUpdate packet with Karma and PvP Flag
		...
		if (maxLoad > 0) {
			...
			if (_curWeightPenalty != newWeightPenalty) {
				...
				if ((newWeightPenalty > 0) && !_dietMode) {
					addSkill(SkillData.getInstance().getSkill(4270,
							newWeightPenalty));
	...
	public void refreshExpertisePenalty() {
		...
		for (L2ItemInstance item : getInventory().getItems()) {
			if ((item != null)
					&& item.isEquipped()
					&& ((item.getItemType() != EtcItemType.ARROW) && (item
							.getItemType() != EtcItemType.BOLT))) {
		...
		if ((getExpertiseWeaponPenalty() != weaponPenalty)
				|| (getSkillLevel(CommonSkill.WEAPON_GRADE_PENALTY.getId()) != weaponPenalty)) {
			_expertiseWeaponPenalty = weaponPenalty;
			if (_expertiseWeaponPenalty > 0) {
				addSkill(SkillData.getInstance().getSkill(
						CommonSkill.WEAPON_GRADE_PENALTY.getId(),
						_expertiseWeaponPenalty));
			} else {
				removeSkill(
						getKnownSkill(CommonSkill.WEAPON_GRADE_PENALTY.getId()),
						false, true);
			}
			changed = true;
		}
		...
		if ((getExpertiseArmorPenalty() != armorPenalty)
				|| (getSkillLevel(CommonSkill.ARMOR_GRADE_PENALTY.getId()) != armorPenalty)) {
			_expertiseArmorPenalty = armorPenalty;
			if (_expertiseArmorPenalty > 0) {
				addSkill(SkillData.getInstance().getSkill(
						CommonSkill.ARMOR_GRADE_PENALTY.getId(),
						_expertiseArmorPenalty));
			} else {
				removeSkill(
						getKnownSkill(CommonSkill.ARMOR_GRADE_PENALTY.getId()),
						false, true);
			}
			changed = true;
		}
	...
	public void useEquippableItem(L2ItemInstance item, boolean abortAttack) {
		...
		if (isEquiped) {
			if (item.getEnchantLevel() > 0) {
				sm = SystemMessage
						.getSystemMessage(SystemMessageId.EQUIPMENT_S1_S2_REMOVED);
				sm.addInt(item.getEnchantLevel());
				sm.addItemName(item);
			} else {
				sm = SystemMessage
						.getSystemMessage(SystemMessageId.S1_DISARMED);
				sm.addItemName(item);
			}
			sendPacket(sm);
			...
			if (slot == L2Item.SLOT_DECO) {
				items = getInventory().unEquipItemInSlotAndRecord(
						item.getLocationSlot());
			} else {
			...
			if (item.isEquipped()) {
				if (item.getEnchantLevel() > 0) {
					sm = SystemMessage
							.getSystemMessage(SystemMessageId.S1_S2_EQUIPPED);
					sm.addInt(item.getEnchantLevel());
					sm.addItemName(item);
				} else {
					sm = SystemMessage
							.getSystemMessage(SystemMessageId.S1_EQUIPPED);
					sm.addItemName(item);
				}
				sendPacket(sm);
		...
		// Notify to scripts
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerEquipItem(this, item), this);
	}
	...
	public void setPvpKills(int pvpKills) {
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerPvPChanged(this, _pvpKills, pvpKills), this);
		_pvpKills = pvpKills;
	}
	...
	public void setFame(int fame) {
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerFameChanged(this, _fame, fame), this);
		_fame = (fame > Config.MAX_PERSONAL_FAME_POINTS) ? Config.MAX_PERSONAL_FAME_POINTS
				: fame;
	}
	...
	public void setClassId(int Id) {
		...
		try {
			if ((getLvlJoinedAcademy() != 0)
					&& (_clan != null)
					&& (PlayerClass.values()[Id].getLevel() == ClassLevel.Third)) {
				if (getLvlJoinedAcademy() <= 16) {
					...
					_clan.addReputationScore(
							(Config.JOIN_ACADEMY_MAX_REP_SCORE - ((getLvlJoinedAcademy() - 16) * 20)),
							true);
				}
				...
				SystemMessage msg = SystemMessage
						.getSystemMessage(SystemMessageId.CLAN_MEMBER_S1_EXPELLED);
				msg.addPcName(this);
				_clan.broadcastToOnlineMembers(msg);
				_clan.broadcastToOnlineMembers(new PledgeShowMemberListDelete(
						getName()));
			...
			if (getClan() != null) {
				getClan().broadcastToOnlineMembers(
						new PledgeShowMemberListUpdate(this));
			}

			// Add AutoGet skills and normal skills and/or learnByFS depending
			// on configurations.
			rewardSkills();
	...
	/**
	 * @param learningClass
	 *            the custom skill learning class Id to set.
	 */
	...
	/**
	 * @param classId
	 * @return the fists weapon of the L2PcInstance Class (used when no weapon
	 *         is equipped).
	 */
	...
	/**
	 * This method reward all AutoGet skills and Normal skills if Auto-Learn
	 * configuration is true.
	 */
	public void rewardSkills() {
		// Give all normal skills if activated Auto-Learn is activated, included
		// AutoGet skills.
	...
	/**
	 * Re-give all skills which aren't saved to database, like Noble, Hero, Clan
	 * Skills.<br>
	 */
	public void regiveTemporarySkills() {
		...
		if (getClan() != null) {
			...
			if ((clan.getLevel() >= SiegeManager.getInstance()
					.getSiegeClanMinLevel()) && isClanLeader()) {
				SiegeManager.getInstance().addSiegeSkills(this);
			}
			if (getClan().getCastleId() > 0) {
				CastleManager.getInstance().getCastleByOwner(getClan())
						.giveResidentialSkills(this);
			}
			if (getClan().getFortId() > 0) {
				FortManager.getInstance().getFortByOwner(getClan())
						.giveResidentialSkills(this);
			}
		}
	 ...
	 * 
	 * @param includedByFs
	 *            if {@code true} forgotten scroll skills present in the skill
	 *            tree will be added
	 * @param includeAutoGet
	 *            if {@code true} auto-get skills present in the skill tree will
	 *            be added
	...
	public int giveAvailableSkills(boolean includedByFs, boolean includeAutoGet) {
		...
		Collection<Skill> skills = SkillTreesData.getInstance()
				.getAllAvailableSkills(this, getClassId(), includedByFs,
						includeAutoGet);
	...
	public void giveAvailableAutoGetSkills() {
		// Get available skills
		final List<L2SkillLearn> autoGetSkills = SkillTreesData.getInstance()
				.getAvailableAutoGetSkills(this);
	...
	public boolean isCastleLord(int castleId) {
		...
		if ((clan != null) && (clan.getLeader().getPlayerInstance() == this)) {
			...
			if ((castle != null)
					&& (castle == CastleManager.getInstance().getCastleById(
							castleId))) {
				return true;
			}
		}
	...
	public int getClanCrestLargeId() {
		if ((_clan != null)
				&& ((_clan.getCastleId() != 0) || (_clan.getHideoutId() != 0))) {
	 ...
	 * send a Server->Client ChangeWaitType packet (broadcast)
	 */
	public void sitDown() {
		sitDown(true);
	}
	...
	public void sitDown(boolean checkCast) {
		...
		if (!_waitTypeSitting && !isAttackingDisabled() && !isOutOfControl()
				&& !isImmobilized()) {
			breakAttack();
			setIsSitting(true);
	 ...
	 * @param sendMessage
	 *            : boolean Specifies whether to send message to Client about
	 *            this action
	 */
	public void addAdena(String process, long count, L2Object reference,
			boolean sendMessage) {
		if (sendMessage) {
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.EARNED_S1_ADENA);
			sm.addLong(count);
			sendPacket(sm);
		}
	 ...
	 */
	public boolean reduceAdena(String process, long count, L2Object reference,
			boolean sendMessage) {
		if (count > getAdena()) {
			if (sendMessage) {
		...
		if (count > 0) {
			...
			if (sendMessage) {
				SystemMessage sm = SystemMessage
						.getSystemMessage(SystemMessageId.S1_DISAPPEARED_ADENA);
				sm.addLong(count);
				sendPacket(sm);
			}
		}
	 ...
	 *            this action
	 */
	public void addAncientAdena(String process, long count, L2Object reference,
			boolean sendMessage) {
		if (sendMessage) {
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.EARNED_S2_S1_S);
	 ...
	 * @return boolean informing if the action was successful
	 */
	public boolean reduceAncientAdena(String process, long count,
		...
		if (count > 0) {
			L2ItemInstance ancientAdenaItem = _inventory
					.getAncientAdenaInstance();
			...
			if (sendMessage) {
				if (count > 1) {
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.S2_S1_DISAPPEARED);
					sm.addItemName(Inventory.ANCIENT_ADENA_ID);
					sm.addLong(count);
					sendPacket(sm);
				} else {
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.S1_DISAPPEARED);
					sm.addItemName(Inventory.ANCIENT_ADENA_ID);
					sendPacket(sm);
				}
			}
	 ...
	 *            this action
	 */
	public void addItem(String process, L2ItemInstance item,
			L2Object reference, boolean sendMessage) {
		if (item.getCount() > 0) {
			...
			if (sendMessage) {
				if (item.getCount() > 1) {
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.YOU_PICKED_UP_S1_S2);
					sm.addItemName(item);
					sm.addLong(item.getCount());
					sendPacket(sm);
				} else if (item.getEnchantLevel() > 0) {
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.YOU_PICKED_UP_A_S1_S2);
					sm.addInt(item.getEnchantLevel());
					sm.addItemName(item);
					sendPacket(sm);
				} else {
					SystemMessage sm = SystemMessage
							.getSystemMessage(SystemMessageId.YOU_PICKED_UP_S1);
					sm.addItemName(item);
					sendPacket(sm);
				}
			}
			...
			L2ItemInstance newitem = _inventory.addItem(process, item, this,
					reference);
			...
			// Combat Flag
				...
				if (FortSiegeManager.getInstance().activateCombatFlag(this,
						item)) {
					Fort fort = FortManager.getInstance().getFort(this);
					fort.getSiege()
							.announceToPlayer(
									SystemMessage
											.getSystemMessage(SystemMessageId.C1_ACQUIRED_THE_FLAG),
									getName());
				}
			}
			// Territory Ward
			else if ((item.getId() >= 13560) && (item.getId() <= 13568)) {
				TerritoryWard ward = TerritoryWarManager.getInstance()
						.getTerritoryWard(item.getId() - 13479);
				if (ward != null) {
	 ...
	 * @return
	 */
	public L2ItemInstance addItem(String process, int itemId, long count,
			L2Object reference, boolean sendMessage) {
		if (count > 0) {
			final L2Item item = ItemTable.getInstance().getTemplate(itemId);
			if (item == null) {
				LOG.error("Item doesn't exist so cannot be added. Item ID: {}",
						itemId);
				return null;
			}
			// Sends message to client if requested
			if (sendMessage
					&& ((!isCastingNow() && item.hasExImmediateEffect()) || !item
							.hasExImmediateEffect())) {
				if (count > 1) {
					if (process.equalsIgnoreCase("Sweeper")
							|| process.equalsIgnoreCase("Quest")) {
						SystemMessage sm = SystemMessage
								.getSystemMessage(SystemMessageId.EARNED_S2_S1_S);
					...
					if (process.equalsIgnoreCase("Sweeper")
							|| process.equalsIgnoreCase("Quest")) {
						SystemMessage sm = SystemMessage
								.getSystemMessage(SystemMessageId.EARNED_ITEM_S1);
						sm.addItemName(itemId);
						sendPacket(sm);
					} else {
						SystemMessage sm = SystemMessage
			...
			if (item.hasExImmediateEffect()) {
				final IItemHandler handler = ItemHandler.getInstance()
						.getHandler(
								item instanceof L2EtcItem ? (L2EtcItem) item
										: null);
				if (handler == null) {
				...
				// Add the item to inventory
				L2ItemInstance createdItem = _inventory.addItem(process,
						itemId, count, this, reference);

				// If over capacity, drop the item
				if (!canOverrideCond(PcCondOverride.ITEM_CONDITIONS)
						&& !_inventory.validateCapacity(0, item.isQuestItem())
						&& createdItem.isDropable()
						&& (!createdItem.isStackable() || (createdItem
								.getLastChange() != L2ItemInstance.MODIFIED))) {
					dropItem("InvDrop", createdItem, null, true);
				} else if (CursedWeaponsManager.getInstance().isCursed(
						createdItem.getId())) {
					CursedWeaponsManager.getInstance().activate(this,
							createdItem);
				}
				// Territory Ward
				else if ((createdItem.getId() >= 13560)
						&& (createdItem.getId() <= 13568)) {
					TerritoryWard ward = TerritoryWarManager.getInstance()
							.getTerritoryWard(createdItem.getId() - 13479);
					if (ward != null) {
	 ...
	 * @param sendMessage
	 *            if {@code true} a system message will be sent
	 */
	public void addItem(String process, ItemHolder item, L2Object reference,
			boolean sendMessage) {
	 ...
	 * @return boolean informing if the action was successful
	 */
	public boolean destroyItem(String process, L2ItemInstance item,
			L2Object reference, boolean sendMessage) {
		return destroyItem(process, item, item.getCount(), reference,
				sendMessage);
	}
	 ...
	 * @return boolean informing if the action was successful
	 */
	public boolean destroyItem(String process, L2ItemInstance item, long count,
			L2Object reference, boolean sendMessage) {
	 ...
	 */
	@Override
	public boolean destroyItem(String process, int objectId, long count,
			L2Object reference, boolean sendMessage) {
	 ...
	 * @return boolean informing if the action was successful
	 */
	public boolean destroyItemWithoutTrace(String process, int objectId,
			long count, L2Object reference, boolean sendMessage) {
		L2ItemInstance item = _inventory.getItemByObjectId(objectId);
	 ...
	 */
	@Override
	public boolean destroyItemByItemId(String process, int itemId, long count,
			L2Object reference, boolean sendMessage) {
		...
		if ((item == null)
				|| (item.getCount() < count)
				|| (_inventory.destroyItemByItemId(process, itemId, count,
						this, reference) == null)) {
			if (sendMessage) {
				sendPacket(SystemMessageId.NOT_ENOUGH_ITEMS);
			}
	 ...
	 *         in inventory
	 */
	public L2ItemInstance transferItem(String process, int objectId,
			long count, Inventory target, L2Object reference) {
		L2ItemInstance oldItem = checkItemManipulation(objectId, count,
				"transfer");
		if (oldItem == null) {
			return null;
		}
		L2ItemInstance newItem = getInventory().transferItem(process, objectId,
				count, target, this, reference);
		if (newItem == null) {
			return null;
		}
	 ...
	 * {@link #addItem(String, L2ItemInstance, L2Object, boolean)} and
	 * {@link #destroyItemByItemId(String, int, long, L2Object, boolean)}<br>
	 * This method validates slots and weight limit, for stackable and
	 * non-stackable items.
	 * 
	 * @param process
	 *            a generic string representing the process that is exchanging
	 *            this items
	 * @param reference
	 *            the (probably NPC) reference, could be null
	 * @param coinId
	 *            the item Id of the item given on the exchange
	 * @param cost
	 *            the amount of items given on the exchange
	 * @param rewardId
	 *            the item received on the exchange
	 * @param count
	 *            the amount of items received on the exchange
	 * @param sendMessage
	 *            if {@code true} it will send messages to the acting player
	 * @return {@code true} if the player successfully exchanged the items,
	 *         {@code false} otherwise
	 */
	public boolean exchangeItemsById(String process, L2Object reference,
			int coinId, long cost, int rewardId, long count, boolean sendMessage) {
	 ...
	 *            item or previous item in transformation
	 * @param sendMessage
	 *            boolean Specifies whether to send message to Client about this
	 *            action
	 * @param protectItem
	 *            whether or not dropped item must be protected temporary
	 *            against other players
	 * @return boolean informing if the action was successful
	 */
	public boolean dropItem(String process, L2ItemInstance item,
			L2Object reference, boolean sendMessage, boolean protectItem) {
		...
		item.dropMe(this, (getX() + Rnd.get(50)) - 25,
				(getY() + Rnd.get(50)) - 25, getZ() + 20);
		...
		if (sendMessage) {
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.YOU_DROPPED_S1);
			sm.addItemName(item);
			sendPacket(sm);
		}
	 ...
	 *         in inventory
	 */
	public L2ItemInstance dropItem(String process, int objectId, long count,
			int x, int y, int z, L2Object reference, boolean sendMessage,
			boolean protectItem) {
		L2ItemInstance invitem = _inventory.getItemByObjectId(objectId);
		L2ItemInstance item = _inventory.dropItem(process, objectId, count,
				this, reference);
	...
	public L2ItemInstance checkItemManipulation(int objectId, long count,
			String action) {
		// TODO: if we remove objects that are not visisble from the L2World,
		// we'll have to remove this check
		if (L2World.getInstance().findObject(objectId) == null) {
			LOG.warn("{} tried to {} item not available in L2World", this,
					action);
			return null;
		}
		...
		if ((count < 0) || ((count > 1) && !item.isStackable())) {
			LOG.warn("{} tried to {} item with invalid count: {}", this,
					action, count);
			return null;
		}
		...
		if ((hasSummon() && (getSummon().getControlObjectId() == objectId))
		...
		if (getActiveEnchantItemId() == objectId) {
			if (Config.DEBUG) {
				LOG.debug("{} tried to {} an enchant scroll he was using",
						this, action);
			}
			return null;
		}

		// We cannot put a Weapon with Augmention in WH while casting (Possible
		// Exploit)
		if (item.isAugmented()
	...
	public void setProtection(boolean protect) {
		if (Config.DEVELOPER && (protect || (_protectEndTime > 0))) {
			LOG.debug(
					...
					this,
					(protect ? "ON "
							+ (GameTimeController.getInstance().getGameTicks() + (Config.PLAYER_SPAWN_PROTECTION * GameTimeController.TICKS_PER_SECOND))
							: "OFF"), GameTimeController.getInstance()
							.getGameTicks());
		}

		_protectEndTime = protect ? GameTimeController.getInstance()
				.getGameTicks()
				+ (Config.PLAYER_SPAWN_PROTECTION * GameTimeController.TICKS_PER_SECOND)
				: 0;
	}
	...
	public void setTeleportProtection(boolean protect) {
		if (Config.DEVELOPER && (protect || (_teleportProtectEndTime > 0))) {
			LOG.debug(
					"{}: Tele Protection {} (currently {})",
					this,
					(protect ? "ON "
							+ (GameTimeController.getInstance().getGameTicks() + (Config.PLAYER_TELEPORT_PROTECTION * GameTimeController.TICKS_PER_SECOND))
							: "OFF"), GameTimeController.getInstance()
							.getGameTicks());
		}

		_teleportProtectEndTime = protect ? GameTimeController.getInstance()
				.getGameTicks()
				+ (Config.PLAYER_TELEPORT_PROTECTION * GameTimeController.TICKS_PER_SECOND)
				: 0;
	}
	...
	public void setRecentFakeDeath(boolean protect) {
		_recentFakeDeathEndTime = protect ? GameTimeController.getInstance()
				.getGameTicks()
	...
	public boolean isRecentFakeDeath() {
		return _recentFakeDeathEndTime > GameTimeController.getInstance()
				.getGameTicks();
	}
	...
	public boolean checkDoCastConditions(Skill skill) {
		...
		if (((getCharges() < skill.getChargeConsume()))
				|| (isInAirShip() && !skill
						.hasEffectType(L2EffectType.REFUEL_AIRSHIP))) {
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.S1_CANNOT_BE_USED);
	...
	private boolean needCpUpdate() {
		...
		if ((currentCp <= _cpUpdateDecCheck)
				|| (currentCp >= _cpUpdateIncCheck)) {
			if (currentCp == getMaxCp()) {
				...
				_cpUpdateDecCheck = _cpUpdateInterval
						* (doubleMulti < intMulti ? intMulti-- : intMulti);
	...
	private boolean needMpUpdate() {
		...
		if ((currentMp <= _mpUpdateDecCheck)
				|| (currentMp >= _mpUpdateIncCheck)) {
			if (currentMp == getMaxMp()) {
				...
				_mpUpdateDecCheck = _mpUpdateInterval
						* (doubleMulti < intMulti ? intMulti-- : intMulti);
	 ...
	 * and only current HP, MP and Level to all other L2PcInstance of the Party.
	 * <B><U> Actions</U> :</B> <li>Send the Server->Client packet StatusUpdate
	 * with current HP, MP and CP to this L2PcInstance</li><BR>
	 * <li>Send the Server->Client packet PartySmallWindowUpdate with current
	 * HP, MP and Level to all other L2PcInstance of the Party</li> <FONT
	 * COLOR=#FF0000><B> <U>Caution</U> : This method DOESN'T SEND current HP
	 * and MP to all L2PcInstance of the _statusListener</B></FONT>
	 */
	@Override
	public void broadcastStatusUpdate() {
		...
		if (isInParty() && (needCpUpdate || needHpUpdate || needMpUpdate())) {
			getParty().broadcastToPartyMembers(this,
					new PartySmallWindowUpdate(this));
		}

		if (isInOlympiadMode() && isOlympiadStart()
				&& (needCpUpdate || needHpUpdate)) {
			final OlympiadGameTask game = OlympiadGameManager.getInstance()
					.getOlympiadTask(getOlympiadGameId());
		...
		if (isInDuel() && (needCpUpdate || needHpUpdate)) {
			DuelManager.getInstance().broadcastToOppositTeam(this,
					new ExDuelUpdateUserInfo(this));
		}
	}

	/**
	 * Send a Server->Client packet UserInfo to this L2PcInstance and CharInfo
	 * to all L2PcInstance in its _KnownPlayers. <B><U> Concept</U> :</B> Others
	 * L2PcInstance in the detection area of the L2PcInstance are identified in
	 * <B>_knownPlayers</B>. In order to inform other players of this
	 * L2PcInstance state modifications, server just need to go through
	 * _knownPlayers to send Server->Client Packet <B><U> Actions</U> :</B> <li>
	 * Send a Server->Client packet UserInfo to this L2PcInstance (Public and
	 * Private Data)</li> <li>Send a Server->Client packet CharInfo to all
	 * L2PcInstance in _KnownPlayers of the L2PcInstance (Public data only)</li>
	 ...
	 * contains PRIVATE DATA as MaxHP, STR, DEX...</B></FONT>
	 */
	public final void broadcastUserInfo() {
		...
		if (TerritoryWarManager.getInstance().isTWInProgress()
				&& (TerritoryWarManager.getInstance().checkIsRegistered(-1,
						getObjectId()) || TerritoryWarManager.getInstance()
						.checkIsRegistered(-1, getClan()))) {
	...
	public final void broadcastPacket(L2GameServerPacket mov) {
		...
		final Collection<L2PcInstance> plrs = getKnownList().getKnownPlayers()
				.values();
		for (L2PcInstance player : plrs) {
			...
			if (mov instanceof CharInfo) {
				...
				Integer oldrelation = getKnownList().getKnownRelations().get(
						player.getObjectId());
				if ((oldrelation != null) && (oldrelation != relation)) {
					player.sendPacket(new RelationChanged(this, relation,
							isAutoAttackable(player)));
					if (hasSummon()) {
						player.sendPacket(new RelationChanged(getSummon(),
								relation, isAutoAttackable(player)));
					}
				}
			}
		}
	...
	public void broadcastPacket(L2GameServerPacket mov, int radiusInKnownlist) {
		...
		for (L2PcInstance player : plrs) {
			...
			if (isInsideRadius(player, radiusInKnownlist, false, false)) {
				...
				if (mov instanceof CharInfo) {
					int relation = getRelation(player);
					Integer oldrelation = getKnownList().getKnownRelations()
	...
	public void queryGameGuard() {
		...
		if (Config.GAMEGUARD_ENFORCE) {
			ThreadPoolManager.getInstance().scheduleGeneral(
					new GameGuardCheckTask(this), 30 * 1000);
		}
	}
	 ...
	 * <li>If the private store is a STORE_PRIVATE_SELL, send a Server->Client
	 * PrivateBuyListSell packet to the L2PcInstance</li> <li>If the private
	 * store is a STORE_PRIVATE_BUY, send a Server->Client PrivateBuyListBuy
	 * packet to the L2PcInstance</li> <li>If the private store is a
	 * STORE_PRIVATE_MANUFACTURE, send a Server->Client RecipeShopSellList
	 * packet to the L2PcInstance</li>
	 * 
	 * @param target
	...
	public void doInteract(L2Character target) {
		if (target instanceof L2PcInstance) {
			...
			if ((temp.getPrivateStoreType() == PrivateStoreType.SELL)
					|| (temp.getPrivateStoreType() == PrivateStoreType.PACKAGE_SELL)) {
	 ...
	 * <li>Send a Server->Client packet InventoryUpdate to this player with
	 * NewItem (use a new slot) or ModifiedItem (increase amount).</li>
	 * <li>Send a Server->Client packet StatusUpdate to this player with current
	 * weight.</li>
	 * </ul>
	 * <font color=#FF0000><B><U>Caution</U>: If a party is in progress,
	 * distribute the items between the party members!</b></font>
	 * 
	 * @param target
	 *            the NPC dropping the item
	 * @param itemId
	 *            the item ID
	 * @param itemCount
	 *            the item count
	...
	/**
	 * Method overload for
	 * {@link L2PcInstance#doAutoLoot(L2Attackable, int, long)}
	 * 
	 * @param target
	 *            the NPC dropping the item
	 * @param item
	...
	/**
	 * Manage Pickup Task. <B><U> Actions</U> :</B> <li>Send a Server->Client
	 * packet StopMove to this L2PcInstance</li> <li>Remove the L2ItemInstance
	 * from the world and send server->client GetItem packets</li> <li>Send a
	 * System Message to the L2PcInstance : YOU_PICKED_UP_S1_ADENA or
	 * YOU_PICKED_UP_S1_S2</li> <li>Add the Item to the L2PcInstance inventory</li>
	 * <li>Send a Server->Client packet InventoryUpdate to this L2PcInstance
	 * with NewItem (use a new slot) or ModifiedItem (increase amount)</li> <li>
	 * Send a Server->Client packet StatusUpdate to this L2PcInstance with
	 * current weight</li> <FONT COLOR=#FF0000><B> <U>Caution</U> : If a Party
	 * is in progress, distribute Items between party members</B></FONT>
	 * 
	 * @param object
	...
	public void doPickupItem(L2Object object) {
		...
		synchronized (target) {
			...
			if (!target.getDropProtection().tryPickUp(this)) {
				sendPacket(ActionFailed.STATIC_PACKET);
				smsg = SystemMessage
						.getSystemMessage(SystemMessageId.FAILED_TO_PICKUP_S1);
				smsg.addItemName(target);
				sendPacket(smsg);
				return;
			}

			if (((isInParty() && (getParty().getDistributionType() == PartyDistributionType.FINDERS_KEEPERS)) || !isInParty())
			...
			if (isInvisible()
					&& !canOverrideCond(PcCondOverride.ITEM_CONDITIONS)) {
				return;
			}

			if ((target.getOwnerId() != 0)
					&& (target.getOwnerId() != getObjectId())
					&& !isInLooterParty(target.getOwnerId())) {
				if (target.getId() == Inventory.ADENA_ID) {
					smsg = SystemMessage
							.getSystemMessage(SystemMessageId.FAILED_TO_PICKUP_S1_ADENA);
					smsg.addLong(target.getCount());
				} else if (target.getCount() > 1) {
					smsg = SystemMessage
							.getSystemMessage(SystemMessageId.FAILED_TO_PICKUP_S2_S1_S);
					smsg.addItemName(target);
					smsg.addLong(target.getCount());
				} else {
					smsg = SystemMessage
			...
			if ((target.getItemLootShedule() != null)
					&& ((target.getOwnerId() == getObjectId()) || isInLooterParty(target
							.getOwnerId()))) {
		...
		if (target.getItem().hasExImmediateEffect()) {
			IItemHandler handler = ItemHandler.getInstance().getHandler(
					target.getEtcItem());
			if (handler == null) {
				LOG.warn("No item handler registered for item ID: {}.",
						target.getId());
			} else {
		...
		// Cursed Weapons are not distributed
			...
			if ((target.getItemType() instanceof ArmorType)
					|| (target.getItemType() instanceof WeaponType)) {
				if (target.getEnchantLevel() > 0) {
					smsg = SystemMessage
							.getSystemMessage(SystemMessageId.ANNOUNCEMENT_C1_PICKED_UP_S2_S3);
					smsg.addPcName(this);
					...
					smsg = SystemMessage
							.getSystemMessage(SystemMessageId.ANNOUNCEMENT_C1_PICKED_UP_S2);
					smsg.addPcName(this);
					smsg.addItemName(target.getId());
					broadcastPacket(smsg, 1400);
				}
			...
			} else if ((target.getId() == Inventory.ADENA_ID)
					&& (getInventory().getAdenaInstance() != null)) {
				addAdena("Pickup", target.getCount(), null, true);
				ItemTable.getInstance().destroyItem("Pickup", target, this,
						null);
			} else {
				addItem("Pickup", target, null, true);
				// Auto-Equip arrows/bolts if player has a bow/crossbow and
				// player picks up arrows/bolts.
				final L2ItemInstance weapon = getInventory().getPaperdollItem(
				...
				if (weapon != null) {
					...
					if (etcItem != null) {
						...
						if ((weapon.getItemType() == WeaponType.BOW)
								&& (itemType == EtcItemType.ARROW)) {
							checkAndEquipArrows();
						} else if ((weapon.getItemType() == WeaponType.CROSSBOW)
								&& (itemType == EtcItemType.BOLT)) {
							checkAndEquipBolts();
						}
					}
				}
	...
	public boolean canOpenPrivateStore() {
		return !isAlikeDead() && !isInOlympiadMode() && !isMounted()
				&& !isInsideZone(ZoneId.NO_STORE) && !isCastingNow();
	}
	...
	public void tryOpenPrivateBuyStore() {
		...
		if (canOpenPrivateStore()) {
			if ((getPrivateStoreType() == PrivateStoreType.BUY)
					|| (getPrivateStoreType() == PrivateStoreType.BUY_MANAGE)) {
	...
	public void tryOpenPrivateSellStore(boolean isPackageSale) {
		...
		// or fake death)
		if (canOpenPrivateStore()) {
			if ((getPrivateStoreType() == PrivateStoreType.SELL)
					|| (getPrivateStoreType() == PrivateStoreType.SELL_MANAGE)
					|| (getPrivateStoreType() == PrivateStoreType.PACKAGE_SELL)) {
				setPrivateStoreType(PrivateStoreType.NONE);
			}
	...
	public void transform(Transform transformation) {
		...
		// Notify to scripts
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerTransform(this, transformation.getId()), this);
	}
	...
	public void untransform() {
		if (_transformation != null) {
			...
			// Notify to scripts
			EventDispatcher.getInstance().notifyEventAsync(
					new OnPlayerTransform(this, 0), this);
		}
	}
	...
	/**
	 * This returns the transformation Id of the current transformation. For
	 * example, if a player is transformed as a Buffalo, and then picks up the
	 * Zariche, the transform Id returned will be that of the Zariche, and NOT
	 * the Buffalo.
	 * 
	 ...
	 * <ul>
	 * <li>Remove the L2PcInstance from the _statusListener of the old target if
	 * it was a L2Character</li>
	 * <li>Add the L2PcInstance to the _statusListener of the new target if it's
	 * a L2Character</li>
	 * <li>Target the new L2Object (add the target to the L2PcInstance _target,
	 * _knownObject and L2PcInstance to _KnownObject of the L2Object)</li>
	 * </ul>
	 * 
	 * @param newTarget
	...
	public void setTarget(L2Object newTarget) {
		if (newTarget != null) {
			boolean isInParty = (newTarget.isPlayer() && isInParty() && getParty()
					.containsPlayer(newTarget.getActingPlayer()));
		...
		if (oldTarget != null) {
			if (oldTarget.equals(newTarget)) // no target change?
			{
				...
				if ((newTarget != null)
						&& (newTarget.getObjectId() != getObjectId())) {
		...
		if (newTarget instanceof L2Character) {
			...
			Broadcast.toKnownPlayers(this, new TargetSelected(getObjectId(),
					newTarget.getObjectId(), getX(), getY(), getZ()));
		}
	...
	public boolean isWearingHeavyArmor() {
		...
		if ((armor != null) && (legs != null)) {
			if ((legs.getItemType() == ArmorType.HEAVY)
					&& (armor.getItemType() == ArmorType.HEAVY)) {
				return true;
			}
		}
		if (armor != null) {
			if (((getInventory().getPaperdollItem(Inventory.PAPERDOLL_CHEST)
					.getItem().getBodyPart() == L2Item.SLOT_FULL_ARMOR) && (armor
					.getItemType() == ArmorType.HEAVY))) {
				return true;
			}
		}
		return false;
	...
	public boolean isWearingLightArmor() {
		...
		if ((armor != null) && (legs != null)) {
			if ((legs.getItemType() == ArmorType.LIGHT)
	...
	public boolean isWearingMagicArmor() {
		...
		if ((armor != null) && (legs != null)) {
			if ((legs.getItemType() == ArmorType.MAGIC)
	...
	public void engageAnswer(int answer) {
		if (!_engagerequest) {
			...
			if (ptarget != null) {
				if (answer == 1) {
					CoupleManager.getInstance().createCouple(ptarget,
							L2PcInstance.this);
	...
	public L2Item getSecondaryWeaponItem() {
		L2ItemInstance item = getInventory().getPaperdollItem(
	 ...
	 * Item Drop. <B><U> Actions</U> :</B> <li>Reduce the Experience of the
	...
	private void onDieDropItem(L2Character killer) {
		...
		if ((getKarma() <= 0) && (pk != null) && (pk.getClan() != null)
				&& (getClan() != null)
		...
		if ((!isInsideZone(ZoneId.PVP) || (pk == null))
				&& (!isGM() || Config.KARMA_DROP_GM)) {
			...
			if ((dropPercent > 0) && (Rnd.get(100) < dropPercent)) {
				...
				for (L2ItemInstance itemDrop : getInventory().getItems()) {
					...
					if (itemDrop.isShadowItem()
							...
							!itemDrop.isDropable()
							|| (itemDrop.getId() == Inventory.ADENA_ID) || // Adena
							(itemDrop.getItem().getType2() == L2Item.TYPE2_QUEST)
							|| // Quest Items
							(hasSummon() && (getSummon().getControlObjectId() == itemDrop
									.getId()))
							|| // Control Item of active pet
							(Arrays.binarySearch(
									Config.KARMA_LIST_NONDROPPABLE_ITEMS,
									itemDrop.getId()) >= 0) || // Item listed in
																// the non
																// droppable
																// item list
							(Arrays.binarySearch(
									Config.KARMA_LIST_NONDROPPABLE_PET_ITEMS,
									itemDrop.getId()) >= 0 // Item listed in the
															// non droppable pet
															// item list
							)) {
						continue;
					...
					if (itemDrop.isEquipped()) {
						// Set proper chance according to Item type of equipped
						// Item
						itemDropPercent = itemDrop.getItem().getType2() == L2Item.TYPE2_WEAPON ? dropEquipWeapon
								: dropEquip;
						getInventory().unEquipItemInSlot(
								itemDrop.getLocationSlot());
					} else {
					...
					if (Rnd.get(100) < itemDropPercent) {
						...
						if (isKarmaDrop) {
							LOG.info(
									"{} has karma and dropped id = {}, count = {}",
									this, itemDrop.getId(), itemDrop.getCount());
						} else {
							LOG.info("{} dropped id = {}, count = {}", this,
									itemDrop.getId(), itemDrop.getCount());
						}
	...
	public void onKillUpdatePvPKarma(L2Character target) {
		...
		if ((checkIfPvP(target) && (targetPlayer.getPvpFlag() != 0))
				|| (isInsideZone(ZoneId.PVP) && targetPlayer
	...
	public void increasePkKillsAndKarma(L2Character target) {
		...
		setKarma(getKarma()
				+ Formulas.calculateKarmaGain(getPkKills(), target.isSummon()));
	...
	public void updatePvPStatus(L2Character target) {
		...
		if ((!isInsideZone(ZoneId.PVP) || !player_target
				.isInsideZone(ZoneId.PVP)) && (player_target.getKarma() == 0)) {
			if (checkIfPvP(player_target)) {
				setPvpFlagLasts(System.currentTimeMillis()
						+ Config.PVP_PVP_TIME);
			} else {
				setPvpFlagLasts(System.currentTimeMillis()
						+ Config.PVP_NORMAL_TIME);
			}
			if (getPvpFlag() == 0) {
				startPvPFlag();
			}
	...
	public boolean isLucky() {
		return (getLevel() <= 9)
				&& isAffectedBySkill(CommonSkill.LUCKY.getId());
	}

	/**
	 * Restore the specified % of experience this L2PcInstance has lost and
	 * sends a Server->Client StatusUpdate packet.
	 * 
	...
	public void restoreExp(double restorePercent) {
		if (getExpBeforeDeath() > 0) {
			...
			getStat()
					.addExp(Math
							.round(((getExpBeforeDeath() - getExp()) * restorePercent) / 100));
			setExpBeforeDeath(0);
		}
	}
	 ...
	 * <B><U> Actions</U> :</B> <li>Calculate the Experience loss</li> <li>Set
	 * the value of _expBeforeDeath</li> <li>Set the new Experience value of the
	 * L2PcInstance and Decrease its level if necessary</li> <li>Send a
	 * Server->Client StatusUpdate packet with its new Experience</li>
	 * 
	 * @param killer
	 * @param atWar
	 */
	public void calculateDeathExpPenalty(L2Character killer, boolean atWar) {
		...
		double percentLost = PlayerXpPercentLostData.getInstance()
				.getXpPercent(getLevel());
		...
		if (!L2Event.isParticipant(this)) {
			if (lvl < ExperienceData.getInstance().getMaxLevel()) {
				lostExp = Math
						.round(((getStat().getExpForLevel(lvl + 1) - getStat()
								.getExpForLevel(lvl)) * percentLost) / 100);
			} else {
				lostExp = Math
						.round(((getStat().getExpForLevel(
								ExperienceData.getInstance().getMaxLevel()) - getStat()
								.getExpForLevel(
										ExperienceData.getInstance()
												.getMaxLevel() - 1)) * percentLost) / 100);
			}
		}
	...
	/**
	 * Stop the HP/MP/CP Regeneration task. <B><U> Actions</U> :</B> <li>Set the
	 * RegenActive flag to False</li> <li>Stop the HP/MP/CP Regeneration task</li>
	 */
	...
	public boolean isProcessingRequest() {
		return (getActiveRequester() != null)
				|| (_requestExpireTime > GameTimeController.getInstance()
						.getGameTicks());
	}
	...
	public void onTransactionRequest(L2PcInstance partner) {
		_requestExpireTime = GameTimeController.getInstance().getGameTicks()
				+ (REQUEST_TIMEOUT * GameTimeController.TICKS_PER_SECOND);
	...
	public boolean isRequestExpired() {
		return !(_requestExpireTime > GameTimeController.getInstance()
				.getGameTicks());
	}
	...
	public void onTradeStart(L2PcInstance partner) {
		...
		SystemMessage msg = SystemMessage
				.getSystemMessage(SystemMessageId.BEGIN_TRADE_WITH_C1);
	...
	public void onTradeConfirm(L2PcInstance partner) {
		SystemMessage msg = SystemMessage
				.getSystemMessage(SystemMessageId.C1_CONFIRMED_TRADE);
		msg.addPcName(partner);
		sendPacket(msg);
	...
	public void onTradeCancel(L2PcInstance partner) {
		...
		SystemMessage msg = SystemMessage
				.getSystemMessage(SystemMessageId.C1_CANCELED_TRADE);
		msg.addPcName(partner);
		sendPacket(msg);
	}
	...
	public Map<Integer, L2ManufactureItem> getManufactureItems() {
		if (_manufactureItems == null) {
			synchronized (this) {
				if (_manufactureItems == null) {
					_manufactureItems = Collections
							.synchronizedMap(new LinkedHashMap<Integer, L2ManufactureItem>());
				}
			}
		}
	 ...
	 * <li>0 : STORE_PRIVATE_NONE</li> <li>1 : STORE_PRIVATE_SELL</li> <li>2 :
	 * sellmanage</li><BR>
	...
	public void setPrivateStoreType(PrivateStoreType privateStoreType) {
		...
		if (Config.OFFLINE_DISCONNECT_FINISHED
				&& (privateStoreType == PrivateStoreType.NONE)
	...
	/**
	 * <B><U> Values </U> :</B> <li>0 : STORE_PRIVATE_NONE</li> <li>1 :
	 * STORE_PRIVATE_SELL</li> <li>2 : sellmanage</li><BR>
	 * <li>3 : STORE_PRIVATE_BUY</li><BR>
	 * <li>4 : buymanage</li><BR>
	 * <li>5 : STORE_PRIVATE_MANUFACTURE</li><BR>
	 * 
	...
	/**
	 * Reduce the number of arrows/bolts owned by the L2PcInstance and send it
	 * Server->Client Packet InventoryUpdate or ItemList (to unequip if the last
	 * arrow was consummed).
	 */
	@Override
	protected void reduceArrowCount(boolean bolts) {
		L2ItemInstance arrows = getInventory().getPaperdollItem(
				Inventory.PAPERDOLL_LHAND);
	 ...
	 * ItemList to the L2PcINstance then return True.
	 */
	@Override
	protected boolean checkAndEquipArrows() {
		...
		if (getInventory().getPaperdollItem(Inventory.PAPERDOLL_LHAND) == null) {
			...
			if (_arrowItem != null) {
				...
				getInventory().setPaperdollItem(Inventory.PAPERDOLL_LHAND,
						_arrowItem);
			...
			_arrowItem = getInventory().getPaperdollItem(
					Inventory.PAPERDOLL_LHAND);
		}
	...
	/**
	 * Equip bolts needed in left hand and send a Server->Client packet ItemList
	 * to the L2PcINstance then return True.
	 */
	@Override
	protected boolean checkAndEquipBolts() {
		...
		if (getInventory().getPaperdollItem(Inventory.PAPERDOLL_LHAND) == null) {
			// Get the L2ItemInstance of the arrows needed for this bow
			_boltItem = getInventory().findBoltForCrossBow(
					getActiveWeaponItem());
			if (_boltItem != null) {
				// Equip arrows needed in left hand
				getInventory().setPaperdollItem(Inventory.PAPERDOLL_LHAND,
			...
			// Get the L2ItemInstance of arrows equiped in left hand
			_boltItem = getInventory().getPaperdollItem(
					Inventory.PAPERDOLL_LHAND);
		}
	 ...
	 * 
	 * @return {@code true} if the player was disarmed or doesn't have a weapon
	 *         to disarm, {@code false} otherwise.
	 */
	public boolean disarmWeapons() {
		// If there is no weapon to disarm then return true.
		final L2ItemInstance wpn = getInventory().getPaperdollItem(
				Inventory.PAPERDOLL_RHAND);
		if (wpn == null) {
			return true;
		}
		...
		final L2ItemInstance[] unequiped = getInventory()
	...
	public boolean disarmShield() {
		L2ItemInstance sld = getInventory().getPaperdollItem(
				Inventory.PAPERDOLL_LHAND);
		if (sld != null) {
			L2ItemInstance[] unequiped = getInventory()
					.unEquipItemInBodySlotAndRecord(sld.getItem().getBodyPart());
			InventoryUpdate iu = new InventoryUpdate();
			for (L2ItemInstance itm : unequiped) {
				iu.addModifiedItem(itm);
			}
	...
	public boolean mountPlayer(L2Summon pet) {
		if ((pet != null) && pet.isMountable() && !isMounted() && !isBetrayed()) {
			...
			if ((getMountType() == MountType.WYVERN)
					&& isInsideZone(ZoneId.NO_LANDING)) {
				sendPacket(ActionFailed.STATIC_PACKET);
	...
	public boolean isInvul() {
		return super.isInvul()
				|| (_teleportProtectEndTime > GameTimeController.getInstance()
						.getGameTicks());
	}
	...
	public void setAccessLevel(int level) {
		...
		if (!AdminData.getInstance().hasAccessLevel(level)) {
			LOG.warn(
					"Tried to set unregistered access level {} for {}. Setting access level without privileges!",
					level, this);
		} else if (level > 0) {
			LOG.info("{} access level set for character {}.",
					_accessLevel.getName(), getName());
		}
	}

	public void setAccountAccesslevel(int level) {
		LoginServerThread.getInstance()
				.sendAccessLevel(getAccountName(), level);
	}
	 ...
	 * L2PcInstance and all L2PcInstance to inform (broadcast).
	 * 
	 * @param flag
	 */
	public void setKarmaFlag(int flag) {
		...
		Collection<L2PcInstance> plrs = getKnownList().getKnownPlayers()
				.values();
		for (L2PcInstance player : plrs) {
			player.sendPacket(new RelationChanged(this, getRelation(player),
					isAutoAttackable(player)));
			if (hasSummon()) {
				player.sendPacket(new RelationChanged(getSummon(),
						getRelation(player), isAutoAttackable(player)));
			}
		}
	}
	 ...
	 * and all L2PcInstance to inform (broadcast).
	 */
	...
	/**
	 * Update the characters table of the database with online status and
	 * lastAccess of this L2PcInstance (called when login and logout).
	 */
	public void updateOnlineStatus() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("UPDATE characters SET online=?, lastAccess=? WHERE charId=?")) {
	...
	private boolean createDb() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
			...
			ps.setLong(36, 0);
			ps.setTimestamp(37,
					new Timestamp(getCreateDate().getTimeInMillis()));
			ps.executeUpdate();
		} catch (Exception e) {
	...
	/**
	 * Retrieve a L2PcInstance from the characters table of the database and add
	 * it in _allObjects of the L2world. <B><U> Actions</U> :</B> <li>Retrieve
	 * the L2PcInstance from the characters table of the database</li> <li>Add
	 * the L2PcInstance object in _allObjects</li> <li>Set the x,y,z position of
	 * the L2PcInstance and make it invisible</li> <li>Update the overloaded
	 * status of the L2PcInstance</li>
	 * 
	 * @param objectId
	 *            Identifier of the object to initialized
	 * @return The L2PcInstance loaded from the database
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
			...
			try (ResultSet rset = ps.executeQuery()) {
				if (rset.next()) {
					...
					player.setClanJoinExpiryTime(rset
							.getLong("clan_join_expiry_time"));
					if (player.getClanJoinExpiryTime() < System
							.currentTimeMillis()) {
						player.setClanJoinExpiryTime(0);
					}
					player.setClanCreateExpiryTime(rset
							.getLong("clan_create_expiry_time"));
					if (player.getClanCreateExpiryTime() < System
							.currentTimeMillis()) {
					...
					player.setFistsWeaponItem(player
							.findFistsWeaponItem(activeClassId));
					...
					// Restore Subclass Data (cannot be done earlier in
					// function)
					if (restoreSubClassData(player)) {
						if (activeClassId != player.getBaseClass()) {
							for (SubClass subClass : player.getSubClasses()
									.values()) {
								if (subClass.getClassId() == activeClassId) {
									player._classIndex = subClass
											.getClassIndex();
								}
							}
						}
					...
					player.setLvlJoinedAcademy(rset
							.getInt("lvl_joined_academy"));
					player.setIsIn7sDungeon(rset.getInt("isin7sdungeon") == 1);

					CursedWeaponsManager.getInstance().checkPlayer(player);

					player.setDeathPenaltyBuffLevel(rset
							.getInt("death_penalty_level"));

					player.setVitalityPoints(rset.getInt("vitality_points"),
							true);

					// Set the x,y,z position of the L2PcInstance and make it
					// invisible
					player.setXYZInvisible(rset.getInt("x"), rset.getInt("y"),
							rset.getInt("z"));
					...
					player.getCreateDate().setTimeInMillis(
							rset.getTimestamp("createDate").getTime());
					...
					if (player.getClan() != null) {
						if (player.getClan().getLeaderId() != player
								.getObjectId()) {
							if (player.getPowerGrade() == 0) {
								player.setPowerGrade(5);
							}
							player.setClanPrivileges(player.getClan()
									.getRankPrivs(player.getPowerGrade()));
						} else {
							player.getClanPrivileges().setAll();
							player.setPowerGrade(1);
						}
						player.setPledgeClass(L2ClanMember
								.calculatePledgeClass(player));
					} else {
						if (player.isNoble()) {
					...
					// Retrieve the name and ID of the other characters assigned
					// to this account.
					try (PreparedStatement stmt = con
							.prepareStatement("SELECT charId, char_name FROM characters WHERE account_name=? AND charId<>?")) {
						stmt.setString(1, player._accountName);
						stmt.setInt(2, objectId);
						try (ResultSet chars = stmt.executeQuery()) {
							while (chars.next()) {
								player._chars.put(chars.getInt("charId"),
										chars.getString("char_name"));
							}
						}
					}
				}
			...
			// L2PcInstance
			// Note that Clan, Noblesse and Hero skills are given separately and
			// not here.
			// Retrieve from the database all skills of this L2PcInstance and
			// add them to _skills
			player.restoreCharData();

			// Reward auto-get skills and all available skills if auto-learn
			// skills is true.
			player.rewardSkills();
			...
			if (player.isGM()) {
				final long masks = player.getVariables().getLong(
						COND_OVERRIDE_KEY,
						PcCondOverride.getAllExceptionsMask());
	...
	public Forum getMail() {
		if (_forumMail == null) {
			setMail(ForumsBBSManager.getInstance().getForumByName("MailRoot")
					.getChildByName(getName()));

			if (_forumMail == null) {
				ForumsBBSManager.getInstance().createNewForum(
						getName(),
						ForumsBBSManager.getInstance().getForumByName(
								"MailRoot"), Forum.MAIL, Forum.OWNERONLY,
						getObjectId());
				setMail(ForumsBBSManager.getInstance()
						.getForumByName("MailRoot").getChildByName(getName()));
			}
		}
	...
	public Forum getMemo() {
		if (_forumMemo == null) {
			setMemo(ForumsBBSManager.getInstance().getForumByName("MemoRoot")
					.getChildByName(_accountName));

			if (_forumMemo == null) {
				ForumsBBSManager.getInstance().createNewForum(
						_accountName,
						ForumsBBSManager.getInstance().getForumByName(
								"MemoRoot"), Forum.MEMO, Forum.OWNERONLY,
						getObjectId());
				setMemo(ForumsBBSManager.getInstance()
						.getForumByName("MemoRoot")
						.getChildByName(_accountName));
			}
		}
	...
	private static boolean restoreSubClassData(L2PcInstance player) {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(RESTORE_CHAR_SUBCLASSES)) {
			...
			try (ResultSet rset = ps.executeQuery()) {
				while (rset.next()) {
					...
					// class indexes.
					player.getSubClasses().put(subClass.getClassIndex(),
							subClass);
				}
			}
		} catch (Exception e) {
	...
	private void restoreCharData() {
		...
		// Retrieve from the database all teleport bookmark of this L2PcInstance
		// and add them to _tpbookmark.
	...
	private void restoreRecipeBook(boolean loadCommon) {
		final String sql = loadCommon ? "SELECT id, type, classIndex FROM character_recipebook WHERE charId=?"
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
			...
			try (ResultSet rset = ps.executeQuery()) {
				while (rset.next()) {
					...
					_premiumItems.put(itemNum, new L2PremiumItem(itemId,
							itemCount, itemSender));
				}
			}
		} catch (Exception e) {
	...
	public void deletePremiumItem(int itemNum) {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("DELETE FROM character_premium_items WHERE charId=? AND itemNum=? ")) {
			ps.setInt(1, getObjectId());
			ps.setInt(2, itemNum);
			ps.execute();
		} catch (Exception e) {
	...
	private void storeCharSub() {
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(UPDATE_CHAR_SUBCLASS)) {
	...
	public void storeEffect(boolean storeEffects) {
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement delete = con
						.prepareStatement(DELETE_SKILL_SAVE);
				PreparedStatement statement = con
						.prepareStatement(ADD_SKILL_SAVE);) {
			...
			if (storeEffects) {
				for (BuffInfo info : getEffectList().getEffects()) {
					...
					final TimeStamp t = getSkillReuseTimeStamp(skill
							.getReuseHashCode());
					statement.setLong(5,
							(t != null) && t.hasNotPassed() ? t.getReuse() : 0);
					statement.setLong(6,
							(t != null) && t.hasNotPassed() ? t.getStamp() : 0);
	...
	private void storeItemReuseDelay() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps1 = con
						.prepareStatement(DELETE_ITEM_REUSE_SAVE);
				PreparedStatement ps2 = con
						.prepareStatement(ADD_ITEM_REUSE_SAVE)) {
	 ...
	 * Normally: The player gets into offline shop mode, only avaiable by
	 * enabling the offline shop mod.
	 * 
	 * @return {@code true} if the player is in offline mode, {@code false}
	 *         otherwise
	 */
	 ...
	 * character_skills table of the database. <B><U> Concept</U> :</B> All
	 * skills own by a L2PcInstance are identified in <B>_skills</B> <B><U>
	 * Actions</U> :</B> <li>Replace oldSkill by newSkill or Add the newSkill</li>
	 * <li>If an old skill has been replaced, remove all its Func objects of
	 * L2Character calculator set</li> <li>Add Func objects of newSkill to the
	 * calculator set of the L2Character</li>
	 * 
	 * @param newSkill
	 ...
	 * the database. <B><U> Concept</U> :</B> All skills own by a L2Character
	 * are identified in <B>_skills</B> <B><U> Actions</U> :</B> <li>Remove the
	 * skill from the L2Character _skills</li> <li>Remove all its Func objects
	 * from the L2Character calculator set</li> <B><U> Overridden in </U> :</B>
	...
	public Skill removeSkill(Skill skill) {
		...
		if (oldSkill != null) {
			try (Connection con = ConnectionFactory.getInstance()
					.getConnection();
					PreparedStatement ps = con
							.prepareStatement(DELETE_SKILL_FROM_CHAR)) {
				// Remove or update a L2PcInstance skill from the
				// character_skills table of the database
	...
	private void storeSkill(Skill newSkill, Skill oldSkill, int newClassIndex) {
		final int classIndex = (newClassIndex > -1) ? newClassIndex
				: _classIndex;
		try (Connection con = ConnectionFactory.getInstance().getConnection()) {
			if ((oldSkill != null) && (newSkill != null)) {
				try (PreparedStatement ps = con
						.prepareStatement(UPDATE_CHARACTER_SKILL_LEVEL)) {
	 ...
	 * 
	 * @param newSkills
	 *            the list of skills to store
	 * @param newClassIndex
	 *            if newClassIndex > -1, the skills will be stored for that
	 *            class index, not the current one
	 */
	...
	private void restoreSkills() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(RESTORE_SKILLS_FOR_CHAR)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					final Skill skill = SkillData.getInstance().getSkill(id,
							level);

					if (skill == null) {
						LOG.warn(
								"Skipped null skill Id: {}, Level: {} while restoring player skills for {}",
								id, level, this);
						continue;
					}
					...
					if (Config.SKILL_CHECK_ENABLE
							&& (!canOverrideCond(PcCondOverride.SKILL_CONDITIONS) || Config.SKILL_CHECK_GM)) {
						if (!SkillTreesData.getInstance().isSkillAllowed(this,
								skill)) {
			...
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					...
					final Skill skill = SkillData.getInstance().getSkill(
							rs.getInt("skill_id"), rs.getInt("skill_level"));
					if (skill == null) {
						continue;
					}
					...
					// Restore Type 1 The remaning skills lost effect upon
					// logout but were still under a high reuse delay.
					if (restoreType > 0) {
						continue;
					}

					// Restore Type 0 These skill were still in effect on the
					// character upon logout.
					// Some of which were self casted and might still have had a
					// long reuse delay which also is restored.
					skill.applyEffects(this, this, false, remainingTime);
				}
			}
			// Remove previously restored skills
			try (PreparedStatement delete = con
					.prepareStatement(DELETE_SKILL_SAVE)) {
				delete.setInt(1, getObjectId());
				delete.setInt(2, getClassIndex());
	...
	/**
	 * Retrieve from the database all Item Reuse Time of this L2PcInstance and
	 * add them to the player.
	 */
	private void restoreItemReuse() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(RESTORE_ITEM_REUSE_SAVE);
				PreparedStatement delete = con
						.prepareStatement(DELETE_ITEM_REUSE_SAVE);) {
			ps.setInt(1, getObjectId());
			try (ResultSet rs = ps.executeQuery()) {
				...
				while (rs.next()) {
					...
					// Using item Id
					L2ItemInstance item = getInventory()
					...
					if ((item != null) && (item.getId() == itemId)
						...
						if (remainingTime > 10) {
							...
							if (isInInventory && item.isEtcItem()) {
								...
								if (group > 0) {
									sendPacket(new ExUseSharedGroupItem(itemId,
											group, (int) remainingTime,
											(int) reuseDelay));
								}
							}
						}
	 ...
	 * _henna and calculate stats of the L2PcInstance.
	 */
	private void restoreHenna() {
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(RESTORE_CHAR_HENNAS)) {
			ps.setInt(1, getObjectId());
			ps.setInt(2, getClassIndex());
			try (ResultSet rset = ps.executeQuery()) {
				int slot;
				...
				while (rset.next()) {
					...
					_henna[slot - 1] = HennaData.getInstance().getHenna(
							symbolId);
				}
			}
		} catch (Exception e) {
	...
	public boolean removeHenna(int slot) {
		...
		getInventory().addItem("Henna", henna.getDyeItemId(),
		...
		// Notify to scripts
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerHennaRemove(this, henna), this);
		return true;
	}
	 ...
	 * 
	 * @param henna
	 *            the henna to add to the player.
	 * @return {@code true} if the henna is added to the player, {@code false}
	 *         otherwise.
	 */
		...
		for (int i = 0; i < 3; i++) {
			if (_henna[i] == null) {
				...
				try (Connection con = ConnectionFactory.getInstance()
						.getConnection();
						PreparedStatement ps = con
								.prepareStatement(ADD_CHAR_HENNA)) {
					ps.setInt(1, getObjectId());
	...
	private void recalcHennaStats() {
		...
		for (L2Henna h : _henna) {
			...
			_hennaINT += ((_hennaINT + h.getStatINT()) > 5) ? 5 - _hennaINT : h
					.getStatINT();
			_hennaSTR += ((_hennaSTR + h.getStatSTR()) > 5) ? 5 - _hennaSTR : h
					.getStatSTR();
			_hennaMEN += ((_hennaMEN + h.getStatMEN()) > 5) ? 5 - _hennaMEN : h
					.getStatMEN();
			_hennaCON += ((_hennaCON + h.getStatCON()) > 5) ? 5 - _hennaCON : h
					.getStatCON();
			_hennaWIT += ((_hennaWIT + h.getStatWIT()) > 5) ? 5 - _hennaWIT : h
					.getStatWIT();
			_hennaDEX += ((_hennaDEX + h.getStatDEX()) > 5) ? 5 - _hennaDEX : h
					.getStatDEX();
		}
	}
	...
	/**
	 * @param slot
	 *            the character inventory henna slot.
	...
	/**
	 * @return {@code true} if player has at least 1 henna symbol, {@code false}
	 *         otherwise.
	 */
	 ...
	 * <li>If the attacker is a L2PcInstance, check if it is not in the same
	 * party</li>
	 * <li>Check if the L2PcInstance has Karma</li>
	 * <li>If the attacker is a L2PcInstance, check if it is not in the same
	 * siege clan (Attacker, Defender)</li>
	 * </ul>
	 */
	@Override
	public boolean isAutoAttackable(L2Character attacker) {
		...
		if (attacker.isPlayable()
				&& (getDuelState() == Duel.DUELSTATE_DUELLING)
				&& (getDuelId() == attacker.getActingPlayer().getDuelId())) {
			return true;
		}

		// Check if the attacker is not in the same party. NOTE: Party checks
		// goes before oly checks in order to prevent patry member autoattack at
		// oly.
		...
		if (attacker.isPlayer()
				&& attacker.getActingPlayer().isInOlympiadMode()) {
			if (isInOlympiadMode()
					&& isOlympiadStart()
		...
		if (attacker.isPlayable()) {
			...
			if (getClan() != null) {
				Siege siege = SiegeManager.getInstance().getSiege(getX(),
						getY(), getZ());
				if (siege != null) {
					// Check if a siege is in progress and if attacker and the
			...
			// Check if the L2PcInstance is in an arena, but NOT siege zone.
			// NOTE: This check comes before clan/ally checks, but after party
			// checks.
			// This is done because in arenas, clan/ally members can autoattack
			// if they arent in party.
			if ((isInsideZone(ZoneId.PVP) && attackerPlayer
					.isInsideZone(ZoneId.PVP))
					&& !(isInsideZone(ZoneId.SIEGE) && attackerPlayer
			...
			if ((getClan() != null)
					&& getClan().isMember(attacker.getObjectId())) {
				return false;
			}

			// Check if the attacker is not in the same ally
			if (attacker.isPlayer() && (getAllyId() != 0)
					&& (getAllyId() == attackerPlayer.getAllyId())) {
				return false;
			}

			// Now check again if the L2PcInstance is in pvp zone, but this time
			// at siege PvP zone, applying clan/ally checks
			if ((isInsideZone(ZoneId.PVP) && attackerPlayer
					.isInsideZone(ZoneId.PVP))
					&& (isInsideZone(ZoneId.SIEGE) && attackerPlayer
							.isInsideZone(ZoneId.SIEGE))) {
	...
	public boolean useMagic(Skill skill, boolean forceUse, boolean dontMove) {
		...
		if (skill.isPassive()) {
			// just ignore the passive skill request. why does the client send
			// it anyway ??
		...
		// If a skill is currently being used, queue this one if this is not the
		// same
		if (isCastingNow()) {
			SkillUseHolder currentSkill = getCurrentSkill();
			// Check if new skill different from current skill in progress
			if ((currentSkill != null)
					&& (skill.getId() == currentSkill.getSkillId())) {
				sendPacket(ActionFailed.STATIC_PACKET);
				return false;
		...
		// This is used mainly to save & queue the button presses, since
		// L2Character has
	...
	private boolean checkUseMagicConditions(Skill skill, boolean forceUse,
			boolean dontMove) {
		// ************************************* Check Player State
		// *******************************************
		...
		if (isFishing()
				&& !skill.hasEffectType(L2EffectType.FISHING,
		...
		if (target.isDoor()) {
			...
			if ((door.getCastle() != null)
					&& (door.getCastle().getResidenceId() > 0)) {
				if (!door.getCastle().getSiege().isInProgress()) {
					sendPacket(SystemMessageId.INCORRECT_TARGET);
					return false;
				}
			} else if ((door.getFort() != null)
					&& (door.getFort().getResidenceId() > 0)) {
				if (!door.getFort().getSiege().isInProgress()
						|| !door.getIsShowHp()) {
					sendPacket(SystemMessageId.INCORRECT_TARGET);
					return false;
				}
		...
		if (isSkillDisabled(skill)) {
			...
			if (hasSkillReuse(skill.getReuseHashCode())) {
				int remainingTime = (int) (getSkillRemainingReuseTime(skill
						.getReuseHashCode()) / 1000);
				...
				if (hours > 0) {
					...
					sm = SystemMessage
							.getSystemMessage(SystemMessageId.S2_MINUTES_S3_SECONDS_REMAINING_FOR_REUSE_S1);
					sm.addSkillName(skill);
					sm.addInt(minutes);
				} else {
					sm = SystemMessage
							.getSystemMessage(SystemMessageId.S2_SECONDS_REMAINING_FOR_REUSE_S1);
					sm.addSkillName(skill);
				}
				...
				sm = SystemMessage
						.getSystemMessage(SystemMessageId.S1_PREPARED_FOR_REUSE);
				sm.addSkillName(skill);
			}
		...
		if (skill.isBad()) {
			...
			if ((target instanceof L2EventMonsterInstance)
					&& ((L2EventMonsterInstance) target)
			...
			if (!target.isAutoAttackable(this) && !forceUse) {
				...
				case UNLOCKABLE:
					break;
				default: // Send a Server->Client packet ActionFailed to the
							// L2PcInstance
					sendPacket(ActionFailed.STATIC_PACKET);
					return false;
				}
			...
			if (dontMove) {
				...
				if (sklTargetType == L2TargetType.GROUND) {
					if (!isInsideRadius(worldPosition.getX(),
						...
						&& !isInsideRadius(target, skill.getCastRange()
								+ getTemplate().getCollisionRadius(), false,
								false)) {
					// Send a System Message to the caster
					sendPacket(SystemMessageId.TARGET_TOO_FAR);
		...
		// Check if the skill is a good magic, target is a monster and if force
		// attack is set, if not then we don't want to cast.
		...
		switch (sklTargetType) {
		case PARTY:
		case CLAN: // For such skills, checkPvpSkill() is called from
					// L2Skill.getTargetList()
		case PARTY_CLAN: // For such skills, checkPvpSkill() is called from
							// L2Skill.getTargetList()
		case AURA:
		case FRONT_AURA:
		case BEHIND_AURA:
		...
		default:
			...
			if (target.isPlayable() && !getAccessLevel().allowPeaceAttack()
					&& !checkPvpSkill(target, skill)) {
		...
		if ((skill.getFlyType() == FlyType.CHARGE)
				&& !GeoData.getInstance().canMove(this, target)) {
	...
	public boolean checkPvpSkill(L2Object target, Skill skill) {
		...
		if (skill.isDebuff()
				|| skill.hasEffectType(L2EffectType.STEAL_ABNORMAL)
				|| skill.isBad()) {
			...
			if (isInParty() && targetPlayer.isInParty()) {
				// Same Party
				if (getParty().getLeader() == targetPlayer.getParty()
						.getLeader()) {
					if ((skill.getEffectRange() > 0) && isCtrlPressed
							&& (getTarget() == target)) {
						if (skill.isDamage()) {
							return true;
						}
					...
					return false;
						...
						&& getParty().getCommandChannel().containsPlayer(
								targetPlayer)) {
					if ((skill.getEffectRange() > 0) && isCtrlPressed
							&& (getTarget() == target)) {
						if (skill.isDamage()) {
			...
			if ((aClan != null) && (tClan != null)) {
				if (aClan.isAtWarWith(tClan.getId())
						&& tClan.isAtWarWith(aClan.getId())) {
					// Check if skill can do damage
					if ((skill.isAOE() && (skill.getEffectRange() > 0))
							&& isCtrlPressed && (getTarget() == target)) {
						return true;
					}
					return isCtrlPressed;
				} else if ((getClanId() == targetPlayer.getClanId())
						|| ((getAllyId() > 0) && (getAllyId() == targetPlayer
								.getAllyId()))) {
					// Check if skill can do damage
					if ((skill.getEffectRange() > 0) && isCtrlPressed
							&& (getTarget() == target) && skill.isDamage()) {
						return true;
					}
					return false;
				}
	...
	public boolean checkLandingState() {
		...
		if (isInsideZone(ZoneId.SIEGE)
				&& !((getClan() != null)
						&& (CastleManager.getInstance().getCastle(this) == CastleManager
								.getInstance().getCastleByOwner(getClan())) && (this == getClan()
	...
	/**
	 * @return the type of Pet mounted (0 : none, 1 : Strider, 2 : Wyvern, 3:
	 *         Wolf).
	 */
	...
	/**
	 * Send a Server->Client packet UserInfo to this L2PcInstance and CharInfo
	 * to all L2PcInstance in its _KnownPlayers.<br>
	 * <B><U>Concept</U>:</B><br>
	 * Others L2PcInstance in the detection area of the L2PcInstance are
	 * identified in <B>_knownPlayers</B>.<br>
	 * In order to inform other players of this L2PcInstance state
	 * modifications, server just need to go through _knownPlayers to send
	 * Server->Client Packet<br>
	 * <B><U>Actions</U>:</B>
	 * <ul>
	 * <li>Send a Server->Client packet UserInfo to this L2PcInstance (Public
	 * and Private Data)</li>
	...
	public void setInventoryBlockingStatus(boolean val) {
		...
		if (val) {
			ThreadPoolManager.getInstance().scheduleGeneral(
					new InventoryEnableTask(this), 1500);
		}
	}
	 ...
	 */
	public L2CubicInstance addCubic(int cubicId, int level, double cubicPower,
			int cubicDelay, int cubicSkillChance, int cubicMaxCount,
			int cubicDuration, boolean givenByOther) {
		return _cubics.put(cubicId, new L2CubicInstance(this, cubicId, level,
				(int) cubicPower, cubicDelay, cubicSkillChance, cubicMaxCount,
				cubicDuration, givenByOther));
	}
			...
			if (item != null) {
				if (magic) {
					if (item.getItem().getDefaultAction() == ActionType.SPIRITSHOT) {
						handler = ItemHandler.getInstance().getHandler(
								item.getEtcItem());
						if (handler != null) {
	...
	public void disableAutoShotByCrystalType(int crystalType) {
		for (int itemId : _activeSoulShots) {
			if (ItemTable.getInstance().getTemplate(itemId).getCrystalType()
					.getId() == crystalType) {
	...
	public boolean disableAutoShot(int itemId) {
		if (_activeSoulShots.contains(itemId)) {
			...
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.AUTO_USE_OF_S1_CANCELLED);
			sm.addItemName(itemId);
			sendPacket(sm);
			return true;
		}
	...
	public void setHero(boolean hero) {
		if (hero && (_baseClass == _activeClass)) {
			for (Skill skill : SkillTreesData.getInstance().getHeroSkillTree()
					.values()) {
				addSkill(skill, false); // Don't persist hero skills into
										// database
			}
		} else {
			for (Skill skill : SkillTreesData.getInstance().getHeroSkillTree()
					.values()) {
				removeSkill(skill, false, true); // Just remove skills from
													// non-hero players
			}
		}
		_hero = hero;
	...
	public boolean canDuel() {
		...
		if (isDead()
				|| isAlikeDead()
				|| ((getCurrentHp() < (getMaxHp() / 2)) || (getCurrentMp() < (getMaxMp() / 2)))) {
		...
		if (isInsideZone(ZoneId.PVP) || isInsideZone(ZoneId.PEACE)
				|| isInsideZone(ZoneId.SIEGE)) {
	...
	public void setNoble(boolean val) {
		final Collection<Skill> nobleSkillTree = SkillTreesData.getInstance()
				.getNobleSkillTree().values();
		if (val) {
	...
	public void sendSkillList() {
		...
		for (Skill s : getAllSkills()) {
			...
			if (((_transformation != null) && !s.isPassive())
					|| (hasTransformSkill(s.getId()) && s.isPassive())) {
				continue;
			}
			if (getClan() != null) {
				isDisabled = s.isClanSkill()
						&& (getClan().getReputationScore() < 0);
			}

			boolean isEnchantable = SkillData.getInstance().isEnchantable(
					s.getId());
			if (isEnchantable) {
				L2EnchantSkillLearn esl = EnchantSkillGroupsData.getInstance()
						.getSkillEnchantmentBySkillId(s.getId());
				if ((esl == null) || (s.getLevel() < esl.getBaseLevel())) {
					isEnchantable = false;
				}
			}

			sl.addSkill(s.getDisplayId(), s.getDisplayLevel(), s.isPassive(),
					isDisabled, isEnchantable);
		}
		if (_transformation != null) {
			Map<Integer, Integer> ts = new TreeMap<>();

			for (SkillHolder holder : _transformation.getTemplate(this)
					.getSkills()) {
			...
			for (AdditionalSkillHolder holder : _transformation.getTemplate(
					this).getAdditionalSkills()) {
			...
			for (L2SkillLearn skill : SkillTreesData.getInstance()
					.getCollectSkillTree().values()) {
				if (getKnownSkill(skill.getSkillId()) != null) {
					addTransformSkill(SkillData.getInstance().getSkill(
							skill.getSkillId(), skill.getSkillLevel()));
				}
			}

			for (Entry<Integer, Integer> transformSkill : ts.entrySet()) {
				Skill sk = SkillData.getInstance().getSkill(
						transformSkill.getKey(), transformSkill.getValue());
				addTransformSkill(sk);
				sl.addSkill(transformSkill.getKey(), transformSkill.getValue(),
						false, false, false);
			}
		}
		sendPacket(sl);
	...
	public boolean addSubClass(int classId, int classIndex) {
		...
		try {
			...
			try (Connection con = ConnectionFactory.getInstance()
					.getConnection();
					PreparedStatement ps = con
							.prepareStatement(ADD_CHAR_SUBCLASS)) {
				...
				LOG.error("Could not add character sub class for {}, {}", this,
						e);
				return false;
			}
			...
			final Map<Integer, L2SkillLearn> skillTree = SkillTreesData
					.getInstance().getCompleteClassSkillTree(subTemplate);
			...
			for (L2SkillLearn skillInfo : skillTree.values()) {
				if (skillInfo.getGetLevel() <= 40) {
					...
					Skill newSkill = SkillData.getInstance().getSkill(
							skillInfo.getSkillId(), skillInfo.getSkillLevel());

					if ((prevSkill != null)
							&& (prevSkill.getLevel() > newSkill.getLevel())) {
						continue;
					}
	 ...
	 * 
	 * @param classIndex
	 *            the class index to delete
	 * @param newClassId
	 *            the new class Id
	 * @return {@code true} if the sub-class was modified, {@code false}
	 *         otherwise
	 */
	public boolean modifySubClass(int classIndex, int newClassId) {
		...
		try {
			try (Connection con = ConnectionFactory.getInstance()
					.getConnection();
					PreparedStatement deleteHennas = con
							.prepareStatement(DELETE_CHAR_HENNAS);
					PreparedStatement deleteShortcuts = con
							.prepareStatement(DELETE_CHAR_SHORTCUTS);
					PreparedStatement deleteSkillReuse = con
							.prepareStatement(DELETE_SKILL_SAVE);
					PreparedStatement deleteSkills = con
							.prepareStatement(DELETE_CHAR_SKILLS);
					PreparedStatement deleteSubclass = con
							.prepareStatement(DELETE_CHAR_SUBCLASS)) {
				...
				LOG.error(
						"Could not modify sub class for {} to class index {}, {}",
						this, classIndex, e);
	...
	private void setClassTemplate(int classId) {
		...
		final L2PcTemplate pcTemplate = PlayerTemplateData.getInstance()
				.getTemplate(classId);
		if (pcTemplate == null) {
		...
		// Notify to scripts
		EventDispatcher.getInstance().notifyEventAsync(
				new OnPlayerProfessionChange(this, pcTemplate,
						isSubClassActive()), this);
	}
	 ...
	 * indexes 1-3 specifies the character's sub-classes respectively.<br>
	...
	public boolean setActiveClass(int classIndex) {
		...
		try {
			...
			if (classIndex == 0) {
				...
				try {
					setClassTemplate(getSubClasses().get(classIndex)
							.getClassId());
				} catch (Exception e) {
					LOG.warn(
							"Could not switch {} sub class to class index {}, {}",
							this, classIndex, e);
					return false;
				}
			}
			...
			// 2. Renovate the characters table in the database with the new
			// class info, storing also buff/effect data.
			// 3. Remove all existing skills.
			// 4. Restore all the learned skills for the current class from the
			// database.
			// 5. Restore effect/buff data for the new class.
			// 6. Restore henna data for the class, applying the new stat
			// modifiers while removing existing ones.
			// 7. Reset HP/MP/CP stats and send Server->Client character status
			// packet to reflect changes.
			// 8. Restore shortcut data related to this class.
			// 9. Resend a class change animation effect to broadcast to all
			// nearby players.
			...
			// Prevents some issues when changing between subclases that shares
			// skills
			resetDisabledSkills();
			...
			broadcastPacket(new SocialAction(getObjectId(),
					SocialAction.LEVEL_UP));
			sendPacket(new SkillCoolTime(this));
			sendPacket(new ExStorageMaxCount(this));

			if (Config.ALTERNATE_CLASS_MASTER) {
				if (Config.CLASS_MASTER_SETTINGS
						.isAllowed(getClassId().level() + 1)
						&& Config.ALTERNATE_CLASS_MASTER
						&& (((this.getClassId().level() == 1) && (this
								.getLevel() >= 40)) || ((this.getClassId()
								.level() == 2) && (this.getLevel() >= 76)))) {
		...
		if (_taskWarnUserTakeBreak == null) {
			_taskWarnUserTakeBreak = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(
							new WarnUserTakeBreakTask(this), 7200000, 7200000);
		}
	}

	public void stopRentPet() {
		if (_taskRentPet != null) {
			// if the rent of a wyvern expires while over a flying zone, tp to
			// down before unmounting
			if (checkLandingState() && (getMountType() == MountType.WYVERN)) {
				teleToLocation(TeleportWhereType.TOWN);
			}

			if (dismount()) // this should always be true now, since we
							// teleported already
			{
	...
	public void startRentPet(int seconds) {
		if (_taskRentPet == null) {
			_taskRentPet = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new RentPetTask(this),
	...
	public void startWaterTask() {
		if (!isDead() && (_taskWater == null)) {
			...
			_taskWater = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(new WaterTask(this),
							timeinwater, 1000);
		}
	}
	...
	public void onPlayerEnter() {
		...
		if (SevenSigns.getInstance().isSealValidationPeriod()
			...
			if (!isGM()
					&& isIn7sDungeon()
					&& (SevenSigns.getInstance().getPlayerCabal(getObjectId()) != SevenSigns
			...
			if (!isGM()
					&& isIn7sDungeon()
					&& (SevenSigns.getInstance().getPlayerCabal(getObjectId()) == SevenSigns.CABAL_NULL)) {
				teleToLocation(TeleportWhereType.TOWN);
				setIsIn7sDungeon(false);
		...
		EventDispatcher.getInstance().notifyEventAsync(new OnPlayerLogin(this),
				this);
	}
	...
	public void doRevive() {
		...
		if (isInParty() && getParty().isInDimensionalRift()) {
			if (!DimensionalRiftManager.getInstance().checkIfInPeaceZone(
					getX(), getY(), getZ())) {
				getParty().getDimensionalRift().memberRessurected(this);
			}
		}
		if (getInstanceId() > 0) {
			final Instance instance = InstanceManager.getInstance()
	...
	public void reviveRequest(L2PcInstance reviver, Skill skill, boolean Pet,
			int power) {
		...
		if (_reviveRequested == 1) {
			if (_revivePet == Pet) {
				reviver.sendPacket(SystemMessageId.RES_HAS_ALREADY_BEEN_PROPOSED); // Resurrection
																					// is
																					// already
																					// been
																					// proposed.
				...
				if (Pet) {
					reviver.sendPacket(SystemMessageId.CANNOT_RES_PET2); // A
																			// pet
																			// cannot
																			// be
																			// resurrected
					...
					reviver.sendPacket(SystemMessageId.MASTER_CANNOT_RES); // While
																			// a
																			// pet
																			// is
																			// attempting
		...
		if ((Pet && hasPet() && getSummon().isDead()) || (!Pet && isDead())) {
			...
			_revivePower = Formulas.calculateSkillResurrectRestorePercent(
					power, reviver);
			restoreExp = (int) Math
					.round(((getExpBeforeDeath() - getExp()) * _revivePower) / 100);
			_revivePet = Pet;

			if (hasCharmOfCourage()) {
				ConfirmDlg dlg = new ConfirmDlg(
						SystemMessageId.RESURRECT_USING_CHARM_OF_COURAGE
								.getId());
				dlg.addTime(60000);
				sendPacket(dlg);
				return;
			...
			ConfirmDlg dlg = new ConfirmDlg(
					SystemMessageId.RESURRECTION_REQUEST_BY_C1_FOR_S2_XP
							.getId());
	...
	public void reviveAnswer(int answer) {
		if ((_reviveRequested != 1) || (!isDead() && !_revivePet)
				|| (_revivePet && hasPet() && !getSummon().isDead())) {
			return;
		}
	...
	public void onActionRequest() {
		if (isSpawnProtected()) {
			...
			if (Config.RESTORE_SERVITOR_ON_RECONNECT
					&& !hasSummon()
					&& CharSummonTable.getInstance().getServitors()
							.containsKey(getObjectId())) {
				CharSummonTable.getInstance().restoreServitor(this);
			}
			if (Config.RESTORE_PET_ON_RECONNECT
					&& !hasSummon()
					&& CharSummonTable.getInstance().getPets()
							.containsKey(getObjectId())) {
	...
	/**
	 * Expertise of the L2PcInstance (None=0, D=1, C=2, B=3, A=4, S=5, S80=6,
	 * S84=7)
	 * 
	...
	public void teleToLocation(ILocational loc, boolean allowRandomOffset) {
		...
		if (isFlyingMounted() && (loc.getZ() < -1005)) {
			super.teleToLocation(loc.getX(), loc.getY(), -1005,
					loc.getHeading(), loc.getInstanceId());
		}
	...
	public void setIsTeleporting(boolean teleport, boolean useWatchDog) {
		...
		if (teleport) {
			if ((_teleportWatchdog == null)
					&& (Config.TELEPORT_WATCHDOG_TIMEOUT > 0)) {
				synchronized (this) {
					if (_teleportWatchdog == null) {
						_teleportWatchdog = ThreadPoolManager
								.getInstance()
								.scheduleGeneral(
										new TeleportWatchdogTask(this),
										Config.TELEPORT_WATCHDOG_TIMEOUT * 1000);
					}
				}
			}
		} else {
	...
	public int getLastServerDistance(int x, int y, int z) {
		return (int) Util.calculateDistance(x, y, z,
				_lastServerPosition.getX(), _lastServerPosition.getY(),
				_lastServerPosition.getZ(), true, false);
	}
	...
	public void reduceCurrentHp(double value, L2Character attacker,
			boolean awake, boolean isDOT, Skill skill) {
		if (skill != null) {
			getStatus().reduceHp(value, attacker, awake, isDOT,
					skill.isToggle(), skill.getDmgDirectlyToHP());
		} else {
	...
	private boolean validateHtmlAction(Iterable<String> actionIter,
			String action) {
		for (String cachedAction : actionIter) {
			if (cachedAction.charAt(cachedAction.length() - 1) == AbstractHtmlPacket.VAR_PARAM_START_CHAR) {
				if (action.startsWith(cachedAction.substring(0,
						cachedAction.length() - 1).trim())) {
					return true;
				}
	 ...
	 * its inventory in the database, Remove it from the world...).<br>
	 * <B><U>Actions</U>:</B>
	 * <ul>
	...
	private synchronized void cleanup() {
			...
			if (getInventory().getItemByItemId(9819) != null) {
				...
				if (fort != null) {
					...
					int slot = getInventory().getSlotFromItem(
							getInventory().getItemByItemId(9819));
					getInventory().unEquipItemInBodySlot(slot);
					destroyItem("CombatFlag",
							getInventory().getItemByItemId(9819), null, true);
				}
			} else if (isCombatFlagEquipped()) {
		...
		try {
			...
			if (_partyroom != 0) {
				PartyMatchRoom room = PartyMatchRoomList.getInstance().getRoom(
						_partyroom);
				if (room != null) {
		...
		if (hasSummon()) {
			try {
				...
				getSummon().unSummon(this);
				// Dead pet wasn't unsummoned, broadcast npcinfo changes (pet
		...
		if (getClan() != null) {
			...
			try {
				L2ClanMember clanMember = getClan()
						.getClanMember(getObjectId());
				if (clanMember != null) {
		...
		try {
			...
			if ((instanceId != 0) && !Config.RESTORE_PLAYER_INSTANCE) {
				final Instance inst = InstanceManager.getInstance()
		...
		if (isCursedWeaponEquipped()) {
			try {
				CursedWeaponsManager.getInstance()
						.getCursedWeapon(_cursedWeaponEquippedId)
						.setPlayer(null);
			} catch (Exception e) {
				LOG.error("deleteMe() {}", e);
			}
		...
		if (getClanId() > 0) {
			getClan().broadcastToOtherOnlineMembers(
					new PledgeShowMemberListUpdate(this), this);
	...
	// fishing zone check.
	// Also worthy of note is the fact the code to find the hook landing
	// position was also striped.
	// The stripped code was moved into fishing.java.
	// In my opinion it makes more sense for it to be there since all other
	// skill related checks were also there.
	// Last but not least, moving the zone check there, fixed a bug where baits
	// would always be consumed no matter if fishing actualy took place.
	// startFishing() now takes up 3 arguments, wich are acurately described as
	// being the hook landing coordinates.
	public void startFishing(int _x, int _y, int _z) {
		...
		// in fishing.
		broadcastPacket(new ExFishingStart(this, _fish.getFishGroup(), _x, _y,
				_z, _lure.isNightLure()));
		...
		if (!isDead() && (_taskforfish == null)) {
			...
			if (_lure != null) {
				...
				if ((lureid == 6519) || (lureid == 6522) || (lureid == 6525)
						|| (lureid == 8505) || (lureid == 8508)
						|| (lureid == 8511)) {
					checkDelay = _fish.getGutsCheckTime() * 133;
				} else if ((lureid == 6520) || (lureid == 6523)
						|| (lureid == 6526)
				...
				} else if ((lureid == 6521) || (lureid == 6524)
						|| (lureid == 6527) || (lureid == 8507)
						|| (lureid == 8510) || (lureid == 8513)) {
					checkDelay = _fish.getGutsCheckTime() * 66;
				}
			}
			_taskforfish = ThreadPoolManager.getInstance()
					.scheduleEffectAtFixedRate(
							new LookingForFishTask(this,
									_fish.getStartCombatTime(),
									_fish.getFishGuts(), _fish.getFishGroup(),
									isNoob, isUpperGrade), 10000, checkDelay);
		}
	}
	...
	private int getRandomFishGroup(int group) {
		...
		case 0: // fish for novices
			switch (_lure.getId()) {
			case 7807: // green lure, preferred by fast-moving (nimble) fish
						// (type 5)
				if (check <= 54) {
					type = 5;
				} else if (check <= 77) {
		...
		case 1: // normal fish
			...
			case 7613:
				type = 3;
				break;
			case 6519: // all theese lures (green) are prefered by fast-moving
						// (nimble) fish (type 1)
			case 8505:
			case 6520:
			case 6521:
			case 8507:
				...
				break;
			case 6522: // all theese lures (purple) are prefered by fat fish
						// (type 0)
			case 8508:
			case 6523:
			case 6524:
			case 8510:
				...
				break;
			case 6525: // all theese lures (yellow) are prefered by ugly fish
						// (type 2)
			case 8511:
			case 6526:
			case 6527:
		...
		case 2: // upper grade fish, luminous lure
			switch (_lure.getId()) {
			case 8506: // green lure, preferred by fast-moving (nimble) fish
						// (type 8)
				if (check <= 54) {
					type = 8;
				} else if (check <= 77) {
	 ...
	 * @param shiftPressed
	 */
	public void setCurrentSkill(Skill currentSkill, boolean ctrlPressed,
			boolean shiftPressed) {
		if (currentSkill == null) {
			_currentSkill = null;
			return;
		}
		_currentSkill = new SkillUseHolder(currentSkill, ctrlPressed,
				shiftPressed);
	}
	 ...
	 * @param shiftPressed
	 */
	public void setCurrentPetSkill(Skill currentSkill, boolean ctrlPressed,
			boolean shiftPressed) {
		if (currentSkill == null) {
			_currentPetSkill = null;
			return;
		}
		_currentPetSkill = new SkillUseHolder(currentSkill, ctrlPressed,
				shiftPressed);
	}
	 ...
	 * @param shiftPressed
	 */
	public void setQueuedSkill(Skill queuedSkill, boolean ctrlPressed,
			boolean shiftPressed) {
		if (queuedSkill == null) {
			_queuedSkill = null;
			return;
		}
		_queuedSkill = new SkillUseHolder(queuedSkill, ctrlPressed,
				shiftPressed);
	}
	...
	public boolean isJailed() {
		return PunishmentManager.getInstance().hasPunishment(getObjectId(),
				PunishmentAffect.CHARACTER, PunishmentType.JAIL)
				|| PunishmentManager.getInstance().hasPunishment(
						getAccountName(), PunishmentAffect.ACCOUNT,
						PunishmentType.JAIL)
				|| PunishmentManager.getInstance().hasPunishment(
						getIPAddress(), PunishmentAffect.IP,
						PunishmentType.JAIL);
	}
	...
	public void startFameTask(long delay, int fameFixRate) {
		...
		if (_fameTask == null) {
			_fameTask = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(
							new FameTask(this, fameFixRate), delay, delay);
		}
	}
	...
	public void startVitalityTask() {
		if (Config.ENABLE_VITALITY && (_vitalityTask == null)) {
			_vitalityTask = ThreadPoolManager.getInstance()
					.scheduleGeneralAtFixedRate(new VitalityTask(this), 1000,
							60000);
		}
	}
	...
	public void increaseSouls(int count) {
		_souls += count;
		SystemMessage sm = SystemMessage
				.getSystemMessage(SystemMessageId.YOUR_SOUL_HAS_INCREASED_BY_S1_SO_IT_IS_NOW_AT_S2);
		sm.addInt(count);
		sm.addInt(_souls);
		sendPacket(sm);
		restartSoulTask();
	...
	private void restartSoulTask() {
		...
		_soulTask = ThreadPoolManager.getInstance().scheduleGeneral(
				new ResetSoulsTask(this), 600000);
	...
	public void calculateDeathPenaltyBuffLevel(L2Character killer) {
		if (killer == null) {
			LOG.warn(
					"{} called calculateDeathPenaltyBuffLevel with killer null!",
					this);
			return;
		}

		if (isResurrectSpecialAffected() || isLucky()
				|| isBlockedFromDeathPenalty() || isInsideZone(ZoneId.PVP)
				|| isInsideZone(ZoneId.SIEGE)
				|| canOverrideCond(PcCondOverride.DEATH_PENALTY)) {
			return;
		}
		double percent = 1.0;
	...
	public void increaseDeathPenaltyBuffLevel() {
		...
		if (getDeathPenaltyBuffLevel() != 0) {
			Skill skill = SkillData.getInstance().getSkill(5076,
					getDeathPenaltyBuffLevel());
		...
		addSkill(
				SkillData.getInstance().getSkill(5076,
						getDeathPenaltyBuffLevel()), false);
		sendPacket(new EtcStatusUpdate(this));
		SystemMessage sm = SystemMessage
				.getSystemMessage(SystemMessageId.DEATH_PENALTY_LEVEL_S1_ADDED);
	...
	@Override
	public final void sendDamageMessage(L2Character target, int damage,
			boolean mcrit, boolean pcrit, boolean miss) {
		// Check if hit is missed
		if (miss) {
			if (target.isPlayer()) {
				SystemMessage sm = SystemMessage
						.getSystemMessage(SystemMessageId.C1_EVADED_C2_ATTACK);
				sm.addPcName(target.getActingPlayer());
				sm.addCharName(this);
				target.sendPacket(sm);
			}
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.C1_ATTACK_WENT_ASTRAY);
			sm.addPcName(this);
			sendPacket(sm);
			return;
		}
		...
		if (pcrit) {
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.C1_HAD_CRITICAL_HIT);
			sm.addPcName(this);
			sendPacket(sm);
		}
		if (mcrit) {
		...
		if (isInOlympiadMode()
				...
				&& (target.getActingPlayer().getOlympiadGameId() == getOlympiadGameId())) {
			OlympiadGameManager.getInstance().notifyCompetitorDamage(this,
					damage);
		}
		...
		if (target.isInvul() && !target.isNpc()) {
			sm = SystemMessage
					.getSystemMessage(SystemMessageId.ATTACK_WAS_BLOCKED);
			...
			sm = SystemMessage
					.getSystemMessage(SystemMessageId.C1_DONE_S3_DAMAGE_TO_C2);
			sm.addPcName(this);
	...
	public void updateVitalityPoints(float points, boolean useRates,
			boolean quiet) {
	...
	public void checkItemRestriction() {
		for (int i = 0; i < Inventory.PAPERDOLL_TOTALSLOTS; i++) {
			...
			if ((equippedItem != null)
					&& !equippedItem.getItem()
							.checkCondition(this, this, false)) {
	...
	protected void startFeed(int npcId) {
		...
		if (hasSummon()) {
			...
			sendPacket(new SetupGauge(3, (getCurrentFeed() * 10000)
					/ getFeedConsume(), (getMaxFeed() * 10000)
					/ getFeedConsume()));
			if (!isDead()) {
				_mountFeedTask = ThreadPoolManager.getInstance()
						.scheduleGeneralAtFixedRate(new PetFeedTask(this),
								10000, 10000);
			}
		} else if (_canFeed) {
	...
	private final L2PetLevelData getPetLevelData(int npcId) {
		if (_leveldata == null) {
			_leveldata = PetDataTable.getInstance().getPetData(npcId)
					.getPetLevelData(getMountLevel());
		}
		return _leveldata;
	}
	...
	public boolean isHungry() {
		return _canFeed ? (getCurrentFeed() < ((PetDataTable.getInstance()
				.getPetData(getMountNpcId()).getHungryLimit() / 100f) * getPetLevelData(
				getMountNpcId()).getPetMaxFeed()))
				: false;
	}

	public void enteredNoLanding(int delay) {
		_dismountTask = ThreadPoolManager.getInstance().scheduleGeneral(
				new DismountTask(this), delay * 1000);
	}
	...
	/**
	 * @param isInHideoutSiege
	 *            sets the value of {@link #_isInHideoutSiege}.
	 */
	...
	/**
	 * @return the value of {@link #_isInHideoutSiege}, {@code true} if the
	 *         player is participing on a Hideout Siege, otherwise {@code false}
	 *         .
	 */
	...
	public void increaseCharges(int count, int max) {
		...
		if (_charges.addAndGet(count) >= max) {
			...
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.FORCE_INCREASED_TO_S1);
	...
	public boolean decreaseCharges(int count) {
		...
		// Charge clear task should be reset every time a charge is decreased
		// and stopped when charges become 0.
	...
	private void restartChargeTask() {
		...
		_chargeTask = ThreadPoolManager.getInstance().scheduleGeneral(
				new ResetChargesTask(this), 600000);
	}
	...
	public void teleportBookmarkModify(int id, int icon, String tag, String name) {
		...
		if (bookmark != null) {
			...
			try (Connection con = ConnectionFactory.getInstance()
					.getConnection();
					PreparedStatement ps = con
							.prepareStatement(UPDATE_TP_BOOKMARK)) {
				ps.setInt(1, icon);
				ps.setString(2, tag);
				ps.setString(3, name);
				...
				LOG.error(
						"Could not update character teleport bookmark data: {}",
						e);
			}
		}
	...
	public void teleportBookmarkDelete(int id) {
		if (_tpbookmarks.remove(id) != null) {
			try (Connection con = ConnectionFactory.getInstance()
					.getConnection();
					PreparedStatement ps = con
							.prepareStatement(DELETE_TP_BOOKMARK)) {
				ps.setInt(1, getObjectId());
				ps.setInt(2, id);
				ps.execute();
			} catch (Exception e) {
				LOG.error(
						"Could not delete character teleport bookmark data: {}",
						e);
			}
	...
	public void teleportBookmarkGo(int id) {
		...
		final TeleportBookmark bookmark = _tpbookmarks.get(id);
		if (bookmark != null) {
			destroyItem("Consume", getInventory().getItemByItemId(13016)
					.getObjectId(), 1, null, false);
	...
	public boolean teleportBookmarkCondition(int type) {
		if (isInCombat()) {
			...
			return false;
				...
				&& (isIn7sDungeon() || (isInParty() && getParty()
						.isInDimensionalRift()))) {
			...
			return false;
				...
				&& (isInsideZone(ZoneId.SIEGE)
						|| isInsideZone(ZoneId.CLAN_HALL)
						|| isInsideZone(ZoneId.JAIL)
						|| isInsideZone(ZoneId.CASTLE)
						|| isInsideZone(ZoneId.NO_SUMMON_FRIEND) || isInsideZone(ZoneId.FORT))) {
			sendPacket(SystemMessageId.YOU_CANNOT_USE_MY_TELEPORTS_TO_REACH_THIS_AREA);
			return false;
		} else if (isInsideZone(ZoneId.NO_BOOKMARK) || isInBoat()
				|| isInAirShip()) {
			if (type == 0) {
		...
		/*
		 * TODO: Instant Zone still not implemented else if
		 * (isInsideZone(ZoneId.INSTANT)) {
		 * sendPacket(SystemMessage.getSystemMessage(2357)); return; }
		 */
		else {
			return true;
		}
	...
	public void teleportBookmarkAdd(int x, int y, int z, int icon, String tag,
			String name) {
		...
		_tpbookmarks
				.put(id, new TeleportBookmark(id, x, y, z, icon, tag, name));

		destroyItem("Consume", getInventory().getItemByItemId(20033)
				.getObjectId(), 1, null, false);
	...
	public void restoreTeleportBookmark() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(RESTORE_TP_BOOKMARK)) {
			ps.setInt(1, getObjectId());
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					_tpbookmarks.put(
							rs.getInt("Id"),
							new TeleportBookmark(rs.getInt("Id"), rs
									.getInt("x"), rs.getInt("y"), rs
									.getInt("z"), rs.getInt("icon"), rs
									.getString("tag"), rs.getString("name")));
				}
			}
		} catch (Exception e) {
	...
	public void sendInfo(L2PcInstance activeChar) {
		if (isInBoat()) {
			...
			if ((oldrelation != null) && (oldrelation != relation1)) {
				activeChar.sendPacket(new RelationChanged(this, relation1,
						isAutoAttackable(activeChar)));
				if (hasSummon()) {
					activeChar.sendPacket(new RelationChanged(getSummon(),
							relation1, isAutoAttackable(activeChar)));
				}
			}
			oldrelation = activeChar.getKnownList().getKnownRelations()
					.get(getObjectId());
			if ((oldrelation != null) && (oldrelation != relation2)) {
				sendPacket(new RelationChanged(activeChar, relation2,
						activeChar.isAutoAttackable(this)));
				if (activeChar.hasSummon()) {
					sendPacket(new RelationChanged(activeChar.getSummon(),
							relation2, activeChar.isAutoAttackable(this)));
				}
			}
			activeChar.sendPacket(new GetOnVehicle(getObjectId(), getBoat()
					.getObjectId(), getInVehiclePosition()));
		} else if (isInAirShip()) {
	...
	public int checkBirthDay() {
		...
		if ((_createDate.get(Calendar.DAY_OF_MONTH) == 29)
				&& (_createDate.get(Calendar.MONTH) == 1)) {
			_createDate.add(Calendar.HOUR_OF_DAY, -24);
		}

		if ((now.get(Calendar.MONTH) == _createDate.get(Calendar.MONTH))
				&& (now.get(Calendar.DAY_OF_MONTH) == _createDate
		...
		for (i = 1; i < 6; i++) {
			...
			if ((now.get(Calendar.MONTH) == _createDate.get(Calendar.MONTH))
					&& (now.get(Calendar.DAY_OF_MONTH) == _createDate
							.get(Calendar.DAY_OF_MONTH))
					&& (now.get(Calendar.YEAR) != _createDate
							.get(Calendar.YEAR))) {
				return i;
			}
		}
	 ...
	 * 
	 * @return the {@code true} if this player is in silence mode, {@code false}
	 *         otherwise
	 */
	 ...
	 * 
	 * @param playerObjId
	 *            the player object Id
	 * @return {@code true} if the given Id is not excluded and this player is
	 *         in silence mode, {@code false} otherwise
	 */
	public boolean isSilenceMode(int playerObjId) {
		if (Config.SILENCE_MODE_EXCLUDE && _silenceMode
				&& (_silenceModeExcluded != null)) {
	...
	public void setSilenceMode(boolean mode) {
		...
		if (_silenceModeExcluded != null) {
			_silenceModeExcluded.clear(); // Clear the excluded list on each
											// setSilenceMode
		}
		sendPacket(new EtcStatusUpdate(this));
	}
	...
	private void storeRecipeShopList() {
		if (hasManufactureShop()) {
			try (Connection con = ConnectionFactory.getInstance()
					.getConnection()) {
				try (PreparedStatement ps = con
						.prepareStatement(DELETE_CHAR_RECIPE_SHOP)) {
					ps.setInt(1, getObjectId());
					ps.execute();
				}

				try (PreparedStatement ps = con
						.prepareStatement(INSERT_CHAR_RECIPE_SHOP)) {
	...
	private void restoreRecipeShopList() {
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement(RESTORE_CHAR_RECIPE_SHOP)) {
			ps.setInt(1, getObjectId());
			try (ResultSet rs = ps.executeQuery()) {
				while (rs.next()) {
					getManufactureItems().put(
							rs.getInt("recipeId"),
							new L2ManufactureItem(rs.getInt("recipeId"), rs
									.getLong("price")));
				}
			}
		} catch (Exception e) {
			LOG.error("Could not restore recipe shop list data for {}, {}",
					this, e);
		}
	}
	...
	public double getCollisionRadius() {
		if (isMounted() && (getMountNpcId() > 0)) {
			return NpcData.getInstance().getTemplate(getMountNpcId())
					.getfCollisionRadius();
		} else if (isTransformed()) {
			return getTransformation().getCollisionRadius(this);
		}
		return getAppearance().getSex() ? getBaseTemplate()
				.getFCollisionRadiusFemale() : getBaseTemplate()
				.getfCollisionRadius();
	}
	...
	public double getCollisionHeight() {
		...
		return getAppearance().getSex() ? getBaseTemplate()
				.getFCollisionHeightFemale() : getBaseTemplate()
				.getfCollisionHeight();
	}
	...
	public final boolean isFalling(int z) {
		if (isDead() || isFlying() || isFlyingMounted()
		...
		// If there is no geodata loaded for the place we are client Z
		// correction might cause falling damage.
		...
		if (damage > 0) {
			reduceCurrentHp(Math.min(damage, getCurrentHp() - 1), null, false,
					true, null);
			SystemMessage sm = SystemMessage
					.getSystemMessage(SystemMessageId.FALL_DAMAGE_S1);
			sm.addInt(damage);
			sendPacket(sm);
		}
	...
	public final void setFalling() {
		_fallingTimestamp = System.currentTimeMillis()
				+ FALLING_VALIDATION_DELAY;
	}
	 ...
	 * skill learn level - 9, skill level is decreased to next possible level.
	 */
	public void checkPlayerSkills() {
		for (Entry<Integer, Skill> e : getSkills().entrySet()) {
			final L2SkillLearn learn = SkillTreesData.getInstance()
					.getClassSkill(e.getKey(), e.getValue().getLevel() % 100,
							getClassId());
			if (learn != null) {
				int lvlDiff = e.getKey() == CommonSkill.EXPERTISE.getId() ? 0
						: 9;
	...
	private void deacreaseSkillLevel(Skill skill, int lvlDiff) {
		int nextLevel = -1;
		final Map<Integer, L2SkillLearn> skillTree = SkillTreesData
				.getInstance().getCompleteClassSkillTree(getClassId());
		for (L2SkillLearn sl : skillTree.values()) {
			if ((sl.getSkillId() == skill.getId())
					&& (nextLevel < sl.getSkillLevel())
					&& (getLevel() >= (sl.getGetLevel() - lvlDiff))) {
		...
		if (nextLevel == -1) {
			...
			addSkill(
					SkillData.getInstance().getSkill(skill.getId(), nextLevel),
					true); // replace with lower one
		}
	}
	...
	public boolean canMakeSocialAction() {
		return ((getPrivateStoreType() == PrivateStoreType.NONE)
				...
				&& !isAllSkillsDisabled() && !isCastingNow()
	...
	public boolean canAttackCharacter(L2Character cha) {
		if (cha instanceof L2Attackable) {
			...
			if (isInDuel() && target.isInDuel()
					&& (target.getDuelId() == getDuelId())) {
				return true;
			} else if (isInParty() && target.isInParty()) {
				if (getParty() == target.getParty()) {
					return false;
				}
				if (((getParty().getCommandChannel() != null) || (target
						.getParty().getCommandChannel() != null))
						&& (getParty().getCommandChannel() == target.getParty()
								.getCommandChannel())) {
					return false;
				}
	...
	private void restorePetInventoryItems() {
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT object_id FROM `items` WHERE `owner_id`=? AND (`loc`='PET' OR `loc`='PET_EQUIP') LIMIT 1;")) {
			ps.setInt(1, getObjectId());
			try (ResultSet rs = ps.executeQuery()) {
				setPetInvItems(rs.next() && (rs.getInt("object_id") > 0));
			}
		} catch (Exception e) {
			LOG.error("Could not check Items in Pet Inventory for {}, {}",
					this, e);
		}
	}
	...
	private long loadRecommendations() {
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("SELECT rec_have,rec_left,time_left FROM character_reco_bonus WHERE charId=? LIMIT 1")) {
			ps.setInt(1, getObjectId());
			try (ResultSet rs = ps.executeQuery()) {
				if (rs.next()) {
	...
	public void storeRecommendations() {
		...
		try (Connection con = ConnectionFactory.getInstance().getConnection();
				PreparedStatement ps = con
						.prepareStatement("INSERT INTO character_reco_bonus (charId,rec_have,rec_left,time_left) VALUES (?,?,?,?) ON DUPLICATE KEY UPDATE rec_have=?, rec_left=?, time_left=?")) {
			ps.setInt(1, getObjectId());
	...
	public void checkRecoBonusTask() {
		...
		_recoGiveTask = ThreadPoolManager.getInstance()
				.scheduleGeneralAtFixedRate(new RecoGiveTask(this), 7200000,
						3600000);
	...
	public void updateNotMoveUntil() {
		_notMoveUntil = System.currentTimeMillis()
				+ Config.PLAYER_MOVEMENT_BLOCK_TIME;
	}
	...
	private final void removeCustomSkill(Skill skill) {
		if ((skill != null) && (_customSkills != null)
				&& (skill.getDisplayId() != skill.getId())) {
			_customSkills.remove(skill.getDisplayId());
		}
	}

	/**
	 * @return {@code true} if current player can revive and shows 'To Village'
	 *         button upon death, {@code false} otherwise.
	 */
	@Override
	...
	/**
	 * @return {@code true} if {@link PlayerVariables} instance is attached to
	 *         current player's scripts, {@code false} otherwise.
	 */
	...
	/**
	 * @return {@link PlayerVariables} instance containing parameters regarding
	 *         player.
	 */
	public PlayerVariables getVariables() {
		final PlayerVariables vars = getScript(PlayerVariables.class);
		return vars != null ? vars : addScript(new PlayerVariables(
				getObjectId()));
	}

	/**
	 * @return {@code true} if {@link AccountVariables} instance is attached to
	 *         current player's scripts, {@code false} otherwise.
	 */
	...
	/**
	 * @return {@link AccountVariables} instance containing parameters regarding
	 *         player.
	 */
	public AccountVariables getAccountVariables() {
		final AccountVariables vars = getScript(AccountVariables.class);
		return vars != null ? vars : addScript(new AccountVariables(
				getAccountName()));
	}
	...
	/**
	 * @param act
	 * @return {@code true} if action was added successfully, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * @param act
	 * @return {@code true} if action was removed successfully, {@code false}
	 *         otherwise.
	 */
	 ...
	 * @param target
	 *            the target
	 * @return {@code true} if this player got war with the target,
	 *         {@code false} otherwise.
	 */
	 ...
	 * @param target
	 *            the target
	 * @return {@code true} if this player in same party with the target,
	 *         {@code false} otherwise.
	 */
	public boolean isInPartyWith(L2Character target) {
		...
		return getParty().getLeaderObjectId() == target.getParty()
				.getLeaderObjectId();
	}
	 ...
	 * @param target
	 *            the target
	 * @return {@code true} if this player in same command channel with the
	...
	public boolean isInCommandChannelWith(L2Character target) {
		...
		if (!getParty().isInCommandChannel()
				|| !target.getParty().isInCommandChannel()) {
			return false;
		}
		return getParty().getCommandChannel().getLeaderObjectId() == target
				.getParty().getCommandChannel().getLeaderObjectId();
	}

	/**
	 * @param target
	 *            the target
	 * @return {@code true} if this player in same clan with the target,
	 *         {@code false} otherwise.
	 */
	 ...
	 * @param target
	 *            the target
	 * @return {@code true} if this player in same ally with the target,
	 *         {@code false} otherwise.
	 */
	 ...
	 * @param target
	 *            the target
	 * @return {@code true} if this player at duel with the target,
	 *         {@code false} otherwise.
	 */
	 ...
	 * @param target
	 *            the target
	 * @return {@code true} if this player is on same siege side with the
	 *         target, {@code false} otherwise.
	 */
	public boolean isOnSameSiegeSideWith(L2Character target) {
		return (getSiegeState() > 0) && isInsideZone(ZoneId.SIEGE)
				&& (getSiegeState() == target.getSiegeState())
				&& (getSiegeSide() == target.getSiegeSide());
	}

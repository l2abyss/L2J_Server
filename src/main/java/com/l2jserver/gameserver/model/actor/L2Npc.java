...
public class L2Npc extends L2Character {
	...
	private int _currentLHandId; // normally this shouldn't change from the
									// template, but there exist exceptions
	private int _currentRHandId; // normally this shouldn't change from the
									// template, but there exist exceptions
	private int _currentEnchant; // normally this shouldn't change from the
									// template, but there exist exceptions
	...
	public L2Npc(L2NpcTemplate template) {
		// Call the L2Character constructor to set the _template of the
		// L2Character, copy skills from template to object
	...
	/**
	 * Verifies if the NPC can cast a skill given the minimum and maximum skill
	 * chances.
	 * 
	 * @return {@code true} if the NPC has chances of casting a skill
	 */
	public boolean hasSkillChance() {
		return Rnd.get(100) < Rnd.get(getTemplate().getMinSkillChance(),
				getTemplate().getMaxSkillChance());
	}
	...
	protected static class RandomAnimationTask implements Runnable {
		private static final Logger LOG = LoggerFactory
				.getLogger(RandomAnimationTask.class);
		...
		public void run() {
			try {
				if (_npc.isMob()) {
					// Cancel further animation timers until intention is
					// changed to ACTIVE again.
				...
				if (!(_npc.isDead() || _npc.isStunned() || _npc.isSleeping() || _npc
						.isParalyzed())) {
				...
				LOG.error(
						"There has been an error trying to perform a random animation for NPC {}!",
						_npc, e);
			}
		}
	}
	 ...
	 * the L2NpcInstance and create a new RandomAnimation Task.
	 * 
	 * @param animationId
	 */
	public void onRandomAnimation(int animationId) {
		// Send a packet SocialAction to all L2PcInstance in the _KnownPlayers
	...
	public boolean hasRandomAnimation() {
		return ((Config.MAX_NPC_ANIMATION > 0) && _isRandomAnimationEnabled && !getAiType()
				.equals(AIType.CORPSE));
	}
	 ...
	 * 
	 * @param val
	 *            needed state of random animation
	 */
	...
	/**
	 * @return {@code true}, if random animation is enabled, {@code false}
	 *         otherwise.
	 */
	...
	/**
	 * @return the Aggro Range of this L2NpcInstance either contained in the
	 ...
	 * in the _KnownPlayers of the L2NpcInstance.
	 */
	@Override
	public void updateAbnormalEffect() {
		// Send a Server->Client packet NpcInfo with state of abnormal effect to
		// all L2PcInstance in the _KnownPlayers of the L2NpcInstance
	 ...
	 *         contained in the L2NpcTemplate.
	 */
	...
	/**
	 * @return the Identifier of the item in the right hand of this
	 *         L2NpcInstance contained in the L2NpcTemplate.
	 */
	...
	/**
	 * @param message
	 *            the busy message of this L2Npc.
	 */
	...
	public final Castle getCastle() {
		...
		if (_castleIndex < 0) {
			...
			if (town != null) {
				_castleIndex = CastleManager.getInstance().getCastleIndex(
						town.getTaxById());
			}

			if (_castleIndex < 0) {
				_castleIndex = CastleManager.getInstance()
						.findNearestCastleIndex(this);
			} else {
	 ...
	 * @param player
	 *            the player to check
	 * @return {@code true} if the player is clan leader and owner of a castle
	 *         of fort that this NPC belongs to, {@code false} otherwise
	 */
	public boolean isMyLord(L2PcInstance player) {
		if (player.isClanLeader()) {
			final int castleId = getCastle() != null ? getCastle()
					.getResidenceId() : -1;
			final int fortId = getFort() != null ? getFort().getResidenceId()
					: -1;
			return (player.getClan().getCastleId() == castleId)
					|| (player.getClan().getFortId() == fortId);
		}
		return false;
	}

	public final SiegableHall getConquerableHall() {
		return CHSiegeManager.getInstance().getNearbyClanHall(getX(), getY(),
				10000);
	}
	...
	public final Castle getCastle(long maxDistance) {
		int index = CastleManager.getInstance().findNearestCastleIndex(this,
				maxDistance);
	...
	public final Fort getFort() {
		...
		if (_fortIndex < 0) {
			Fort fort = FortManager.getInstance().getFort(getX(), getY(),
					getZ());
			if (fort != null) {
				_fortIndex = FortManager.getInstance().getFortIndex(
						fort.getResidenceId());
			}

			if (_fortIndex < 0) {
				_fortIndex = FortManager.getInstance().findNearestFortIndex(
						this);
			}
		}
	...
	public final Fort getFort(long maxDistance) {
		int index = FortManager.getInstance().findNearestFortIndex(this,
				maxDistance);
	...
	public void onBypassFeedback(L2PcInstance player, String command) {
		// if (canInteract(player))
		{
			if (isBusy() && (getBusyMessage().length() > 0)) {
				...
				IBypassHandler handler = BypassHandler.getInstance()
						.getHandler(command);
				if (handler != null) {
					handler.useBypass(command, player, this);
				} else {
					LOG.info("Unknown NPC bypass: \"{}\" NpcId: {}", command,
							getId());
				}
			}
		}
	...
	/**
	 * Return the weapon item equipped in the right hand of the L2NpcInstance or
	 * null.
	 */
	@Override
	public L2Weapon getActiveWeaponItem() {
		...
		L2Item item = ItemTable.getInstance().getTemplate(
				getTemplate().getRHandId());
	...
	/**
	 * Return the weapon item equipped in the left hand of the L2NpcInstance or
	 * null.
	 */
	@Override
	 ...
	 * to display the message of the L2NpcInstance.
	 * 
	 * @param player
	 ...
	 * <ul>
	 * <li>if the file exists on the server (page number = 0) :
	 * <B>data/html/default/12006.htm</B> (npcId-page number)</li>
	 * <li>if the file exists on the server (page number > 0) :
	 * <B>data/html/default/12006-1.htm</B> (npcId-page number)</li>
	 * <li>if the file doesn't exist on the server :
	 * <B>data/html/npcdefault.htm</B> (message :
	 * "I have nothing to say to you")</li>
	 * </ul>
	 * 
	 * @param npcId
	...
	private boolean showPkDenyChatWindow(L2PcInstance player, String type) {
		final String html = HtmCache.getInstance().getHtm(
				player.getHtmlPrefix(),
				"data/html/" + type + "/" + getId() + "-pk.htm");
		if (html != null) {
	 ...
	 * <B><U>Actions</U>:</B>
	 * <ul>
	 * <li>Get the text of the selected HTML file in function of the npcId and
	 ...
	 * <li>Send a Server->Client ActionFailed to the L2PcInstance in order to
	 * avoid that the client wait another packet</li>
	 * </ul>
	 * 
	 * @param player
	...
	public void showChatWindow(L2PcInstance player, int val) {
		...
		if (player.isCursedWeaponEquipped()
				&& (!(player.getTarget() instanceof L2ClanHallManagerInstance) || !(player
		...
		if (player.getKarma() > 0) {
			if (!Config.ALT_GAME_KARMA_PLAYER_CAN_SHOP
					&& (this instanceof L2MerchantInstance)) {
			...
			} else if (!Config.ALT_GAME_KARMA_PLAYER_CAN_USE_WAREHOUSE
					&& (this instanceof L2WarehouseInstance)) {
				if (showPkDenyChatWindow(player, "warehouse")) {
					return;
				}
			} else if (!Config.ALT_GAME_KARMA_PLAYER_CAN_SHOP
					&& (this instanceof L2FishermanInstance)) {
		...
		int sealAvariceOwner = SevenSigns.getInstance().getSealOwner(
				SevenSigns.SEAL_AVARICE);
		int sealGnosisOwner = SevenSigns.getInstance().getSealOwner(
				SevenSigns.SEAL_GNOSIS);
		int playerCabal = SevenSigns.getInstance().getPlayerCabal(
				player.getObjectId());
		...
		case 36402:
			if (player.getOlympiadBuffCount() > 0) {
				filename = (player.getOlympiadBuffCount() == Config.ALT_OLY_MAX_BUFFS ? Olympiad.OLYMPIAD_HTML_PATH
						+ "olympiad_buffs.htm"
		...
		if (this instanceof L2MerchantInstance) {
			if (Config.LIST_PET_RENT_NPC.contains(npcId)) {
				html.replace("_Quest",
						"_RentPet\">Rent Pet</a><br><a action=\"bypass -h npc_%objectId%_Quest");
			}
		}

		html.replace("%objectId%", String.valueOf(getObjectId()));
		html.replace("%festivalMins%", SevenSignsFestival.getInstance()
				.getTimeToNextFestivalStr());
		player.sendPacket(html);
	 ...
	 * name and path, relative to the datapack root.
	 * 
	 * @param player
	 *            The L2PcInstance that talk with the L2NpcInstance
	 * @param filename
	 ...
	 * <B><U>Actions</U>:</B>
	 * <ul>
	 * <li>Create a DecayTask to remove the corpse of the L2NpcInstance after 7
	 * seconds</li>
	 ...
	 * all other L2PcInstance to inform</li>
	...
	public boolean doDie(L2Character killer) {
		...
		// normally this wouldn't really be needed, but for those few
		// exceptions,
		...
		final L2Weapon weapon = (killer != null) ? killer.getActiveWeaponItem()
				: null;
	...
	public void onSpawn() {
		...
		if (isTeleporting()) {
			EventDispatcher.getInstance().notifyEventAsync(
					new OnNpcTeleport(this), this);
		} else {
			EventDispatcher.getInstance().notifyEventAsync(
					new OnNpcSpawn(this), this);
		}
	 ...
	 * a complete removal use the deleteMe method).<br>
	 * <B><U>Actions</U>:</B>
	 * <ul>
	 * <li>Remove the L2NpcInstance from the world when the decay task is
	 * launched</li>
	...
	public void onDecay() {
		...
		// Remove the L2NpcInstance from the world when the decay task is
		// launched
		super.onDecay();
	 ...
	 * L2NpcInstance then cancel Attack or Cast and notify AI</li>
	...
	public boolean deleteMe() {
		...
		// L2Character then cancel Attack or Cast and notify AI
		try {
	...
	{
		return false; // This means we use MAX_NPC_ANIMATION instead of
						// MAX_MONSTER_ANIMATION
	}

	// Two functions to change the appearance of the equipped weapons on the NPC
	// This is only useful for a few NPCs and is most likely going to be called
	// from AI
	...
	public void sendInfo(L2PcInstance activeChar) {
		if (isVisibleFor(activeChar)) {
			...
			if (getRunSpeed() == 0) {
				...
				activeChar.sendPacket(new AbstractNpcInfo.NpcInfo(this,
						activeChar));
			}
		}
	}
	...
	public void showNoTeachHtml(L2PcInstance player) {
		...
		if (this instanceof L2WarehouseInstance) {
			html = HtmCache.getInstance().getHtm(player.getHtmlPrefix(),
					"data/html/warehouse/" + npcId + "-noteach.htm");
		} else if (this instanceof L2TrainerInstance) {
			html = HtmCache.getInstance().getHtm(player.getHtmlPrefix(),
					"data/html/trainer/" + npcId + "-noteach.htm");
			// Trainer Healer?
			if (html == null) {
				html = HtmCache.getInstance().getHtm(
						player.getHtmlPrefix(),
						"data/scripts/ai/npc/Trainers/HealerTrainer/" + npcId
								+ "-noteach.html");
			}
		}
		...
		if (html == null) {
			...
			noTeachMsg
					.setHtml("<html><body>I cannot teach you any skills.<br>You must find your current class teachers.</body></html>");
		} else {
	...
	@Override
	protected final void notifyQuestEventSkillFinished(Skill skill,
			L2Object target) {
		if ((target != null) && target.isPlayable()) {
			EventDispatcher.getInstance().notifyEventAsync(
					new OnNpcSkillFinished(this, target.getActingPlayer(),
							skill), this);
		}
	}
	...
	public boolean isMovementDisabled() {
		return super.isMovementDisabled() || !canMove()
				|| getAiType().equals(AIType.CORPSE);
	}
		...
		if ((_soulshotamount > 0) || (_spiritshotamount > 0)) {
			if (physical) {
				...
				Broadcast.toSelfAndKnownPlayersInRadius(this,
						new MagicSkillUse(this, this, 2154, 1, 0, 0), 600);
			...
			if (magic) {
				...
				_spiritshotamount--;
				Broadcast.toSelfAndKnownPlayersInRadius(this,
						new MagicSkillUse(this, this, 2061, 1, 0, 0), 600);
	 ...
	 * @param val
	 *            value to store
	 * @return {@code true} if stored script value equals given value,
	...
	public int getAIValue(final String paramName) {
		return hasAIValue(paramName) ? NpcPersonalAIData.getInstance()
				.getAIValue(getSpawn().getName(), paramName) : -1;
	}

	/**
	 * @param paramName
	 *            the parameter name to check
	 * @return {@code true} if given parameter is set for NPC, {@code false}
	 *         otherwise
	 */
	public boolean hasAIValue(final String paramName) {
		return (getSpawn() != null)
				&& (getSpawn().getName() != null)
				&& NpcPersonalAIData.getInstance().hasAIValue(
						getSpawn().getName(), paramName);
	}

	/**
	 * @param npc
	 *            NPC to check
	 * @return {@code true} if both given NPC and this NPC is in the same spawn
	 *         group, {@code false} otherwise
	 */
	public boolean isInMySpawnGroup(L2Npc npc) {
		return ((getSpawn() != null) && (npc.getSpawn() != null)
				&& (getSpawn().getName() != null) && (getSpawn().getName()
				.equals(npc.getSpawn().getName())));
	}

	/**
	 * @return {@code true} if NPC currently located in own spawn point,
	 *         {@code false} otherwise
	 */
	public boolean staysInSpawnLoc() {
		return ((getSpawn() != null) && (getSpawn().getX(this) == getX()) && (getSpawn()
				.getY(this) == getY()));
	}

	/**
	 * @return {@code true} if {@link NpcVariables} instance is attached to
	...
	/**
	 * @return {@link NpcVariables} instance containing parameters regarding
	 *         NPC.
	 */
	 ...
	 *            - name of event
	 * @param radius
	 *            - radius to send event
	 * @param reference
	 *            - L2Object to pass, if needed
	 */
	public void broadcastEvent(String eventName, int radius, L2Object reference) {
		for (L2Object obj : L2World.getInstance().getVisibleObjects(this,
				radius)) {
			if (obj.isNpc() && obj.hasListener(EventType.ON_NPC_EVENT_RECEIVED)) {
				EventDispatcher.getInstance().notifyEventAsync(
						new OnNpcEventReceived(eventName, this, (L2Npc) obj,
								reference), obj);
			}
		}
	}
	 ...
	 *            the reference
	 */
	public void sendScriptEvent(String eventName, L2Object receiver,
			L2Object reference) {
		EventDispatcher.getInstance().notifyEventAsync(
				new OnNpcEventReceived(eventName, this, (L2Npc) receiver,
						reference), receiver);
	}
	 ...
	 * 
	 * @param radiusMin
	 *            miminal range from NPC (not closer than)
	 * @param radiusMax
	 *            maximal range from NPC (not further than)
	...
	public Location getPointInRange(int radiusMin, int radiusMax) {
		...
		return new Location((int) (getX() + (radius * Math.cos(angle))),
				(int) (getY() + (radius * Math.sin(angle))), getZ());
	}
	 ...
	 * 
	 * @param player
	 *            the last attacker or main damage dealer
	 * @param itemId
	 *            the item ID
	 * @param itemCount
	 *            the item count
	 ...
	 */
	public L2ItemInstance dropItem(L2PcInstance player, int itemId,
			long itemCount) {
		L2ItemInstance item = null;
		for (int i = 0; i < itemCount; i++) {
			// Randomize drop position.
			final int newX = (getX() + Rnd
					.get((RANDOM_ITEM_DROP_LIMIT * 2) + 1))
					- RANDOM_ITEM_DROP_LIMIT;
			final int newY = (getY() + Rnd
					.get((RANDOM_ITEM_DROP_LIMIT * 2) + 1))
					- RANDOM_ITEM_DROP_LIMIT;
			...
			if (ItemTable.getInstance().getTemplate(itemId) == null) {
				LOG.error(
						"Item doesn't exist so cannot be dropped. Item ID: {} Quest: {}",
						itemId, getName());
				return null;
			}

			item = ItemTable.getInstance().createItem("Loot", itemId,
					itemCount, player, this);
			if (item == null) {
				return null;
			}
			...
			if (!Config.LIST_PROTECTED_ITEMS.contains(itemId)) {
				if (((Config.AUTODESTROY_ITEM_AFTER > 0) && !item.getItem()
						.hasExImmediateEffect())
						|| ((Config.HERB_AUTO_DESTROY_TIME > 0) && item
								.getItem().hasExImmediateEffect())) {
					ItemsAutoDestroy.getInstance().addItem(item);
				}
			}
			item.setProtected(false);

			// If stackable, end loop as entire count is included in 1 instance
			// of item.
	...
	/**
	 * Method overload for
	 * {@link L2Attackable#dropItem(L2PcInstance, int, long)}
	 * 
	 * @param player
	 *            the last attacker or main damage dealer
	 * @param item
	...
	public boolean isVisibleFor(L2PcInstance player) {
		if (hasListener(EventType.ON_NPC_CAN_BE_SEEN)) {
			final TerminateReturn term = EventDispatcher.getInstance()
					.notifyEvent(new OnNpcCanBeSeen(this, player), this,
	 ...
	 * 
	 * @param val
	 *            {@code true} if the players can talk, {@code false} otherwise
	 */
	...
	public final Collection<L2Npc> getSummonedNpcs() {
		return _summonedNpcs != null ? _summonedNpcs.values() : Collections
				.<L2Npc> emptyList();
	}

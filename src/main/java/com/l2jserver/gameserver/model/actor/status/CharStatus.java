...
public class CharStatus {
	 ...
	 * all L2PcInstance to inform of HP/MP updates.<br>
	 * Players who must be informed are players that target this L2Character.<br>
	 * When a RegenTask is in progress sever just need to go through this list
	 * to send Server->Client packet StatusUpdate.<br>
	 ...
	 * Players who must be informed are players that target this L2Character.<br>
	 * When a RegenTask is in progress sever just need to go through this list
	 * to send Server->Client packet StatusUpdate.
	 * 
	...
	public void reduceHp(double value, L2Character attacker, boolean awake,
			boolean isDOT, boolean isHPConsumption) {
		...
		if (attacker != null) {
			...
			if ((attackerPlayer != null) && attackerPlayer.isGM()
	...
	public final synchronized void startHpMpRegeneration() {
		if ((_regTask == null) && !getActiveChar().isDead()) {
			...
			_regTask = ThreadPoolManager.getInstance()
	 ...
	 *            if true StatusUpdate packet will be broadcasted.
	...
	protected void doRegeneration() {
		...
		// Modify the current HP of the L2Character and broadcast Server->Client
		// packet StatusUpdate
		if (getCurrentHp() < charstat.getMaxRecoverableHp()) {
			setCurrentHp(
					getCurrentHp() + Formulas.calcHpRegen(getActiveChar()),
					false);
		}

		// Modify the current MP of the L2Character and broadcast Server->Client
		// packet StatusUpdate
		if (getCurrentMp() < charstat.getMaxRecoverableMp()) {
			setCurrentMp(
					getCurrentMp() + Formulas.calcMpRegen(getActiveChar()),
					false);
		}
		...
		if (getActiveChar().isInActiveRegion()) {
			getActiveChar().broadcastStatusUpdate(); // send the StatusUpdate
														// packet
		}
	}

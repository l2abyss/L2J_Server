...
public abstract class AbstractNpcInfo extends L2GameServerPacket {
	...
	public AbstractNpcInfo(L2Character cha) {
		...
		_swimWalkSpd = (int) Math.round(cha.getSwimWalkSpeed()
				/ _moveMultiplier);
	...
	public static class NpcInfo extends AbstractNpcInfo {
		...
		public NpcInfo(L2Npc cha, L2Character attacker) {
			...
			if ((cha instanceof L2NpcInstance)
					&& cha.isInsideZone(ZoneId.TOWN)
					&& (Config.SHOW_CREST_WITHOUT_QUEST || cha.getCastle()
							.getShowNpcCrest())
					&& (cha.getCastle().getOwnerId() != 0)) {
				int townId = TownManager.getTown(_x, _y, _z).getTownId();
				if ((townId != 33) && (townId != 22)) {
					L2Clan clan = ClanTable.getInstance().getClan(
							cha.getCastle().getOwnerId());
		...
		protected void writeImpl() {
			...
			writeC(_isSummoned ? 2 : 0); // invisible ?? 0=false 1=true
											// 2=summoned (only works if model
											// has a summon animation)
			...
			writeD(_npc.isInvisible() ? _npc.getAbnormalVisualEffects()
					| AbnormalVisualEffect.STEALTH.getMask() : _npc
			...
			writeC(_npc.isInsideZone(ZoneId.WATER) ? 1 : _npc.isFlying() ? 2
					: 0); // C2
			...
			writeD(0x00);
			writeD(_npc.getColorEffect()); // CT1.5 Pet form and skills, Color
											// effect
			writeC(_npc.isTargetable() ? 0x01 : 0x00);
			writeC(_npc.isShowName() ? 0x01 : 0x00);

			writeD(_npc.isChampion() && Config.ELITE_VISIBLE_AURA ? AbnormalVisualEffect.NAVIT_ADVENT
					.getMask() : _npc.getAbnormalVisualEffectSpecial());
	...
	public static class TrapInfo extends AbstractNpcInfo {
		...
		protected void writeImpl() {
			...
			writeD(_trap.isInvisible() ? _trap.getAbnormalVisualEffects()
					| AbnormalVisualEffect.STEALTH.getMask() : _trap
					.getAbnormalVisualEffects());
			writeD(0x00); // clan id
	...
	public static class SummonInfo extends AbstractNpcInfo {
		...
		public SummonInfo(L2Summon cha, L2Character attacker, int val) {
			...
			_name = cha.getName();
			_title = (cha.getOwner() != null) && cha.getOwner().isOnline() ? cha
					.getOwner().getName() : "";
			_idTemplate = cha.getTemplate().getDisplayId();
		...
		protected void writeImpl() {
			...
			if (isInvisible()) {
				...
				if ((activeChar != null)
						&& activeChar
								.canOverrideCond(PcCondOverride.SEE_ALL_PLAYERS)) {
					gmSeeInvis = true;
				}
			}
			...
			writeC(_val); // invisible ?? 0=false 1=true 2=summoned (only works
							// if model has a summon animation)
			writeD(-1); // High Five NPCString ID
			writeS(_name);
			writeD(-1); // High Five NPCString ID
			writeS(_title);
			...
			writeD(gmSeeInvis ? _summon.getAbnormalVisualEffects()
					| AbnormalVisualEffect.STEALTH.getMask() : _summon
					.getAbnormalVisualEffects());
			...
			writeD(0x00); // C2
			writeD(0x00); // C2
			writeC(_summon.isInsideZone(ZoneId.WATER) ? 1
					: _summon.isFlying() ? 2 : 0); // C2
